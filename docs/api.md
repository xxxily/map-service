...
后端会校验图层、区域、缩放级别、并发数和请求间隔。默认最大并发为 64，可通过服务端配置调整。`maxTiles` 只作为建议上限，超出上限的任务仍允许创建，前端应展示风险提示。`requestIntervalMs` 表示任意两个瓦片请求开始之间的最小间隔，默认 0，最大 60000。
...
- `deleteCache=true`：同时删除该任务 provider、bounds 和 zoom 范围对应的 fetch-relay 瓦片缓存文件及元数据。执行中的任务会先停止，随后清理关联缓存。
