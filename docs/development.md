...
如果媒体预览是从地图媒体点位直接打开且入口 popup 已关闭，首次导航即使仍落在同一要素，也要按延迟激活该要素；从 popup 媒体入口打开时继续跳过同要素重建。
资源集合面板打开统一媒体预览时也必须传递同一活动项回调；集合项虽然共享一个 Point 要素，但直接入口的首次切换仍需恢复概要 popup，后续同要素切换不得重复重建。
