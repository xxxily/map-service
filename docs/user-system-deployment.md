...
服务启动时自动创建数据库目录、启用外键、设置 `busy_timeout`、尝试切换 WAL，并按 `schema_migrations` 执行幂等迁移。当前数据库版本为 `9`：v2～v4 完善 KML 同步幂等和删除墓碑；v5 增加空间受限分享与不限固定期授权；v6 增加手动同步的发布快照；v7 增加访问聚合、统计配置和动态限流字段；v8 增加分享密码的所有者专用加密密文；v9 增加 KML 目录、目录内文件排序及分享目录引用字段。
