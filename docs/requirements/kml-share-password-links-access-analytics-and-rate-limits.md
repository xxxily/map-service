...
- 分享者复制带密码链接或单独复制密码时不再重复输入密码；所有者接口在 `no-store` 响应中短暂返回密码。加密副本缺失或无法解密时接口保守失败，测试分享直接删除重建或重新设置密码，不增加旧版本自动恢复逻辑。
- 自动生成密码使用浏览器 CSPRNG，默认 12 位；用户可选择 8、12、16、20、24 或 32 位。
- 生成器默认包含大小写字母、数字和特殊字符；关闭特殊字符后只使用大小写字母和数字。特殊字符限定为 `!$*+@`，主动排除 `?`、`&`、`#`、`%`、`=` 等 URL 查询分隔符，减少复制和人工转交时的歧义。
- 密码放入链接前必须使用 `encodeURIComponent` 编码，即使未来字符集调整也不能把密码内容解释成额外查询参数；密码只显示在当前对话中，不持久化。
...
- 生成密码控件提供长度和“包含特殊字符”选项，默认 12 位且勾选特殊字符；用户手工输入的分享密码仍由服务端按 4～128 位规则校验。
- 管理员允许无密码分享时，创建对话框提供“不设置密码”，编辑对话框提供“移除密码”；管理员关闭后这两个操作由服务端拒绝，前端不能绕过该策略。
- 无密码分享可以将 `expiresAt` 设置为 `null`，但密码授权模式为 `not_applicable`，不能选择 `unlimited`；`unlimited` 只适用于已设置密码且空间受限、范围合规的分享。
...
3. 当前没有正式用户，本次密码和分享策略调整不提供旧分享、旧链接或旧范围的兼容迁移；测试分享删除后按新规则重建。旧密码密文缺失也不作为新实现的兼容分支。
4. 数据库已有 `password_secret` 字段继续用于新密码的所有者复制，不在普通接口、日志和审计中返回。
5. 通用数据库迁移仍需幂等；本需求不新增为旧测试分享保留行为的迁移逻辑。

## 7. 验收标准

- 带密码链接首次打开可自动进入分享地图；修改密码后旧链接重新要求输入且不能绕过。
- 页面启动后地址栏、后续浏览历史、Referer 和统计脚本请求中不出现密码；生产网关日志按运维要求隐藏首次请求的查询参数。
- 生成密码满足所选长度和字符集要求，默认 12 位；关闭特殊字符后不出现 `!$*+@`，且生成结果不包含 `?& #%=` 等查询分隔符；服务端只存哈希并用编码后的值生成安全链接。
