...
- 服务端继续以密码哈希和 `password_version` 作为访问验证事实；同时使用稳定部署密钥保存 AES-256-GCM 密文，仅用于分享所有者主动复制密码或带密码链接。普通分享接口、公开接口、管理治理接口、日志和审计元数据均不得返回该密文或密码。
...
- 分享者复制带密码链接或单独复制密码时不再重复输入密码；所有者接口在 `no-store` 响应中短暂返回密码。历史版本只有哈希、无法恢复明文的分享，需重新设置一次密码后才能使用复制能力。
...
- 用户中心“复制链接”提供“普通链接”“带密码链接”和“复制密码”三个选项。
- 无密码分享不显示带密码选项。
...
4. v8 为 `kml_shares` 增加 `password_secret` 密文列；旧分享默认留空，重新设置密码后补齐。
5. 迁移必须幂等；密文只允许在所有者主动复制接口中解密，禁止进入常规读取响应、日志和审计。
