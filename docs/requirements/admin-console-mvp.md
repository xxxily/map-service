...
- 后台每个子页必须有可刷新的路由状态，刷新后仍停留在当前子页，例如 `/admin/precache`。
...
- 已创建任务支持再次编辑：把任务的区域、图层和级别回填到创建表单，允许调整后提交为新的任务。
- 任务支持更新操作：对任务范围内瓦片执行非强制更新，已有缓存命中时跳过，新鲜度过期时按条件请求更新。
- 任务列表默认展示摘要信息，详细 bounds、级别明细、错误等内容应折叠展示，避免窄宽度下信息拥挤。
...
- 前端后台 tab 路由必须与页面模块对应，新增子页时同步维护路由映射。
...
- 未登录访问后台展示登录页。
- 登录成功后能进入后台并通过导航切换面板。
- 后台每个子页刷新后仍保持当前 tab，例如 `/admin/precache` 刷新后仍显示预缓存页。
...
- 已完成任务可以点击编辑回填表单，调整后再次创建任务；已有缓存瓦片默认跳过，缺失瓦片继续下载。
- 点击更新任务时不会强制刷新所有瓦片，只对过期或缺失缓存执行正常缓存更新流程。
- 任务列表在有限宽度下保持清晰，详细信息通过展开查看。
- 图层页能查看所有图层配置。
