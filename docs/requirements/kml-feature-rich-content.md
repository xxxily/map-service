...
- 视频使用稳定的 `16:9` 可用区域、`playsinline` 和原生控制条；普通 `<video>` 资源默认不自动播放，`embed` / `object` 识别为视频后进入预览即自动播放，若带声音播放被浏览器拦截则静音重试。直接支持浏览器可播放的视频 URL，并为 HLS `.m3u8` 提供受控播放适配。`embed` / `object` 资源可通过 `type="video/*"`、视频扩展名或 KML `MarkerStyleVideo` 提示识别为视频，加载失败时保留原始视频入口。
