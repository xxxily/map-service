...
状态：第一阶段已实现链接及受支持 HTML 媒体标签解析、公开点位内容接口、媒体点位图标、前台详情面板和统一媒体预览器；当前交互升级为“点位首击预览、整个 KML 连续浏览、全屏与小窗切换”。第三方应用分享文本的 provider 识别与受控 iframe 转换见 `kml-point-share-link-embed.md`，第二阶段内容库集成规划见 `kml-rich-content-phase2-content-library.md`。后续新增接口仍必须同步更新 `docs/api.md`、相关后端测试和前端交互说明。
...
- 不把富媒体二进制、内容库记录或后台配置写入标准 KML 导出文件；可移植的描述链接和 provider iframe 标签仍可作为 `description` 文本保存。
...
- 导出时继续输出标准 KML，不包含内容库、后台 iframe 白名单配置或服务端内部 ID；用户描述及 provider 生成的可移植媒体标签仍作为标准 `description` 文本转义保存。
...
- 有真实点位名称时展示名称；空名称或历史“未命名点位/未命名要素 N”占位名不在轻量 popup 的标题、媒体卡片或无障碍标签中重复展示，详情面板仍使用未命名兜底。
- 有文字描述时展示最多 2 到 4 行摘要；只有媒体而没有描述时不增加“包含可预览媒体”等重复提示。
...
- popup 媒体区只显示媒体数量，不额外提示“可浏览整个 KML”；连续浏览能力由统一预览器的轨道和切换控件直接表达。
...
- 每次切换活动媒体都同步激活地图中的所属点位并打开其 popup；关闭查看器后地图保留最后一次浏览媒体对应的点位 popup，不额外提供与实时激活能力重复的点位跳转按钮。
- 媒体轨道中的图片使用懒加载缩略图，视频、音频和页面使用轻量类型封面；不得因为打开查看器而一次性加载整个 KML 的原始媒体。
- 支持将全屏查看器收缩为可继续播放和切换的浮动小窗，并可重新展开；小窗状态解除页面滚动锁且不得阻断地图交互。
- 桌面全屏模式下顶部与底部控制层默认透明度为 0.3，鼠标进入或键盘焦点落入对应区域时恢复为 1；移动端隐藏顶部、底部信息层，仅保留主媒体区域、关闭、前后切换和小窗等核心操作。
...
- 图片首次显示时以预览区域的可用高宽为基准等比自适应并居中，比例为 `1x`；小于预览区域的图片也应放大适配，而不是按原始像素尺寸显示。加载完成、失败、横竖屏变化或图片切换不能造成默认位置偏移。
...
- 视频使用稳定的 `16:9` 可用区域、`playsinline` 和原生控制条；普通 `<video>`、`embed` 和 `object` 识别得到的视频进入预览后都自动播放，优先尝试带声音播放，若被浏览器策略拦截则静音重试。直接支持浏览器可播放的视频 URL，并为 HLS `.m3u8` 提供受控播放适配。`embed` / `object` 资源可通过 `type="video/*"`、视频扩展名或 KML `MarkerStyleVideo` 提示识别为视频，加载失败时保留原始视频入口。
