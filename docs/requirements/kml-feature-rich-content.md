...
KML 点位交互按输入环境分流，桌面鼠标保持轻量 popup 路径，触屏移动环境优先减少进入媒体预览的点击次数：

- 桌面鼠标环境点击 Point 点位继续打开轻量 popup，用户从缩略图或类型卡片进入统一媒体预览器，现有操作顺序不变。
- 触屏优先环境中，带可预览媒体的 Point 点位单击直接打开统一媒体预览器；长按至少 650ms 且移动距离不超过 10px 时打开原轻量 popup，用于查看简介、详情、编辑、删除或收藏等操作。
- 触屏环境中的无媒体 Point、LineString 和 Polygon 继续使用原有 popup；窄窗口但使用精细鼠标指针的桌面设备不得被误判为移动交互。
- 2D Leaflet、3D Cesium、个人 KML、公共 KML 和只读分享 KML 使用同一交互语义；长按不得同时触发媒体预览、地图新增点位或浏览器上下文菜单。

轻量 popup 应承担“快速判断”和操作入口职责：
...
- 点击缩略图或类型卡片直接进入统一媒体预览器；桌面从点位到大图/播放器最多两次点击，触屏媒体点位单击一次即可进入。
...
- 提供可滚动的媒体轨道，显示类型、所属点位、当前位置和总数；点击轨道项、上一项/下一项按钮或方向键均可切换，首尾循环。只有一项媒体时不显示轨道和开关。
- 桌面端每次打开预览器时媒体轨道默认展开，用户可通过图标收起；触屏移动环境每次打开时默认收起，用户可按需展开。收起后轨道不得继续占用媒体显示高度，竖屏和横屏都应一致。
- 桌面端切换活动媒体继续同步激活地图中的所属点位并打开其 popup；触屏全屏预览不在遮罩后反复打开 popup，用户关闭预览后可通过长按点位进入简介操作。
...
- 打开时聚焦媒体查看区域并锁定页面滚动，`Escape`、遮罩和关闭按钮均可退出；关闭后恢复触发元素焦点，不能出现无法关闭或焦点落到地图背后的状态。
- 触屏移动环境打开预览器时应压入一个同 URL 历史哨兵。浏览器返回按钮或系统返回手势只消费该哨兵并关闭预览，不退出地图页；使用关闭按钮退出时也必须回收自身哨兵，不能留下额外一次无效返回。
...
- 桌面 2D/3D 点击媒体点位先打开 popup，再点击媒体进入大图或播放器；触屏 2D/3D 单击媒体点位直接预览，长按打开 popup。
- 2D 点位详情打开、关闭、切换点位。
- 3D 点位详情打开、关闭、切换点位。
- 图片加载失败、视频播放失败、iframe 被拒绝。
- 移动端底部面板布局。
- 桌面与移动端统一预览器的打开、关闭、焦点恢复和滚动锁定。
- 移动端媒体轨道默认收起、图标展开/收起、收起后空间释放，以及桌面轨道默认展开。
- 移动端浏览器返回按钮和返回手势只关闭预览，关闭按钮回收历史哨兵，随后再次打开仍可正常响应。
...
- 桌面用户第一次点击含媒体的 KML 点位可看到缩略预览，第二次点击媒体进入查看器；触屏用户单击含媒体 Point 直接进入查看器，长按仍可打开简介和操作入口。
