...
- 管理员可配置个人地图全局 Point 聚合，默认开启且单网格最少 10 点；分享页始终以分享级配置为准，全局设置不得覆盖分享显式关闭。
...
4. 分享公开 manifest 的聚合配置和分享页 2D/3D 点位聚合。
5. 管理员个人地图全局聚合配置，以及个人 KML 在 2D/3D 中的跨文件 Point 聚合。
6. SQLite 迁移、输入校验、权限检查、审计、接口文档和自动化测试。
...
新增 `directoryId` 和 `position` 字段。`position` 只在同一目录（含未分类）的 active 文件内比较；回收站记录不占顺序槽位。排序接口采用完整 ID 顺序提交，服务端重新编号，避免客户端稀疏序号和并发冲突。数据库 schema v11 按 `ownerId + directoryId` 修复历史 active 顺序，并把 trashed 位置归零。默认 KML仍受默认文件保护，但可以显示在未分类目录中。
...
管理员个人地图全局配置位于 `settings.kml.pointClustering`：
...
该配置通过 `/api/v1/auth/config` 脱敏下发，作用于个人地图中所有 active、enabled 且已加载的 KML，跨文件聚合 Point。分享页检测到 active share 后必须忽略该配置。
...
- 全局聚合开启时，2D/3D 将所有个人 KML 的 Point 汇总后按当前缩放级别计算，不按单文件分别形成重叠聚合。
- 文件显隐、点位增删改、目录操作和视口变化触发一次整体聚合重绘；聚合点点击后使用动画逐级放大。
- LineString/Polygon 始终使用原完整渲染路径；聚合不得固定数量省略文件、点位名称、媒体或管理列表内容。
- 打开分享链接时停止使用全局配置；分享 `enabled=false`、分享自身参数和管理员分享强制策略按分享契约处理。
...
`GET/PUT /api/v1/admin/user-system/settings` 使用 `kml.pointClustering` 六个字段；`GET /api/v1/auth/config` 返回相同的脱敏渲染字段。管理员接口要求 `admin.security.manage`，并校验缩放范围、网格、最少点数和成员明细上限。
...
5. 全局聚合默认开启且最少 10 点，个人 2D/3D 跨文件 Point 聚合；分享显式关闭优先于全局设置。点击聚合可逐级放大，LineString/Polygon、媒体 popup 和连续缩放不受影响。
6. 完成 `npm run check`、`npm test`、`npm run build`；若环境阻断必须记录具体命令和原因。

## 8. 实施顺序

1. SQLite schema v11、纯函数 normalize/排序/聚合算法和服务层测试。
2. REST 路由、API 文档和个人 KML/分享服务契约。
3. 账户页和分享对话框目录管理、文件移动排序和显隐。
4. 地图 KML 面板目录渲染、文件拖拽与目录批量显隐。
5. 个人地图与分享页 2D/3D 点位聚合、性能/连续缩放验证。
6. 回归、构建和开发记录。
