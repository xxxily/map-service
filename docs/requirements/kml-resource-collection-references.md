...
> 状态：核心能力已在 v1.5.67 实现并完成 161 测试与 66 生产环境验收；v1.5.63 的历史部署记录见 [map-service v1.5.63 资源集合引用发布记录](../../../notes/serverinfo/161_notes/OperationLog/2026/09/04/map-service-v1.5.63-resource-collection-reference-deploy.md)。
>
> 日期：2026-09-05
>
...
- KML `ExtendedData` 导入导出、同账号重绑定和跨账号未解析状态；内嵌/独立集合互转列为后续版本，不影响本期引用能力。
...
- 个人空间集合详情已补齐单项添加/编辑/删除、跨页排序、批量添加、回收站只读和空集合首项入口；列表、详情、来源选择器和移动端布局已统一整理。
- 个人集合和引用分页响应共用严格 schema 校验，异常页码、总数、版本、重复 ID、缺失或非 HTTPS 地址不会被静默当作空数据；公开确认和永久删除前会展示引用影响。
- 外部引用继续使用浏览器直读，失败时提供重试、复制安全原地址和权限状态提示；原有内嵌集合数据路径、KML 分享和动态公开权限保持兼容。
- 内嵌集合另存为个人集合、个人集合转回内嵌集合及转换时 300 项/512 KiB 限制尚未实现，明确作为后续 Phase 5 事项。
- v1.5.67 已完成 161/66 部署；版本、健康检查、数据库校验、备份路径和回滚证据见两台服务器的操作记录。

- v1.5.67 修复个人集合选择器分页往返误报、3D 状态点清理、外部项稳定 ID 校验和可配置批量上限分块；已有个人/外部绑定时需先解除绑定，再选择内嵌模式，避免无提示丢失远程集合内容。
- v1.5.67 的部署记录同时覆盖本次边界修复后的最终生产版本；后续变更仍须沿用同一备份、回滚和公开权限验收流程。

## 17. 后续事项与风险

以下事项不影响 v1.5.67 已实现能力，作为后续版本的决策与风险跟踪项：
...
8. 内嵌集合另存为个人集合、个人集合转回内嵌集合，以及转换时的 300 项/512 KiB 限制；本版本保留两种数据模式并存，后续实现转换时必须显式确认体积和引用影响。
