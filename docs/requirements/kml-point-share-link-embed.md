...
状态：第一阶段已实现，已完成需求示例的桌面端和移动端浏览器回归，待用户环境验收。本文保留抖音 provider 的专项基线；720 云 provider 与点位图标选择已在 [KML 点位 720 云内容与可配置图标](./kml-720yun-and-marker-icons.md) 中实现并单独维护。
...
720 云公开 `/vr/<ID>`、`/t/<ID>` 地址不需要服务端展开，按专项需求在浏览器本地规范化并生成受控 iframe；解析接口只处理声明为需要服务端展开的短链（当前为抖音短链）。
...
- iframe 使用与预览舞台一致的深色表面，水平方向必须占满预览舞台，不设置固定最大宽度和左右留白；仅当 iframe 高度小于可用高度时进行上下居中。
- provider 可以生成只用于前端加载的自适应预览地址。抖音播放器使用 `width=100vw`、`height=calc(100vh + 48px)` 适配 iframe 视口并把官方附加页脚置于可视区域之外，播放器主体和控制条仍完整保留；持久化到 KML 的标准播放器 URL 仍只包含稳定视频 ID。
- 点位 popup 的媒体速览入口必须支持重复打开。关闭统一媒体预览后再次点击同一媒体卡片，应立即重新打开，不得要求先关闭并重开点位 popup。
...
720 云地址（例如 `https://www.720yun.com/vr/f4ejtOsf5y0`）会在保存前由共享 provider 适配器本地转换为 `provider=720yun` 的 iframe，不调用本接口、不回源读取作品数据；其 `resourceId` 使用 `vr:<ID>` 或 `t:<ID>`，两种路径不互相推导。
...
12. iframe 在桌面和移动端均与预览舞台同宽，只保留上下居中能力，不出现左右居中造成的空隙。
13. 从点位媒体速览打开预览、关闭后，再次点击同一媒体卡片可以连续重复打开；切换到同一要素的其他媒体不会重建当前 popup。
