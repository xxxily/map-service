...
- 新批次调用同步接口前，客户端必须先保存 `pendingOperations` 并等待 IndexedDB 完整草稿写入成功；写入失败时不得发送请求。成功响应只有在逐条校验 `results` 与请求的 action、`clientId/kmlId`、`document/result` 身份后才清空在途批次并归并结果；畸形 2xx 响应视为 `KML_SYNC_RESPONSE_INCOMPLETE`，pending 必须保留且不得自动递归请求。普通网络/HTTP 错误保存最新草稿，并从当前工作集重建 pending 操作；下一次显式 flush 或新编辑才重试。若请求在途期间发生新编辑，重试批次以最新工作集为准，不继续发送过时的旧批次。
...
- 工作集缺少文件本身不是删除意图。只有显式删除、确认批量删除或重做删除才写入 `deletedFileIds/deletedClientIds` 和 `deletionIntent`；服务端对每一条 `trash` 都要求 `user-confirmed` 或 `user-confirmed-batch`，否则返回 `409 KML_DELETE_CONFIRMATION_REQUIRED`。旧草稿缺少确认意图时，恢复流程忽略其删除操作并从服务端 active 清单补回文件。
