...
所有图源、派生矢量资源和代理测试目标统一经过 `service/bin/security/networkTarget.js`：配置阶段拒绝非 HTTP 协议、凭据 URL、内部域名和字面保留地址，并统一关闭重定向。直连与远程代理解析全部 DNS 结果并固定已验证地址，同时保留原 HTTP `Host`、HTTPS SNI 和证书校验主机名；混合 DNS、私网和保留地址继续拒绝。

明确配置为 `127.0.0.0/8`、`::1` 或 `localhost` 的本机 HTTP/HTTPS 代理属于受控信任边界：对非字面 IP 的公开目标，由该代理解析原始域名，避免 Clash/Mihomo Fake-IP 或运营商 DNS 污染被服务端误判或固定到错误 IP。该能力只由管理员配置的图源/代理诊断调用，公开瓦片接口仍只能提交稳定图源 ID，不能选择代理或传入任意 URL；URL 字面内网地址、内部域名、直连和远程代理不适用此例外。
