...
function renderCollections (state) {
  ...
  const itemTotalCandidate = collections.itemResult?.total ?? collections.itemResult?.pagination?.total ?? collections.itemTotal
  const itemTotal = Number.isSafeInteger(Number(itemTotalCandidate)) && Number(itemTotalCandidate) >= 0
    ? Number(itemTotalCandidate)
    : null
  const itemTotalPages = itemTotal === null ? Math.max(1, Number(collections.itemPageCount || 1)) : Math.max(1, Math.ceil(itemTotal / itemLimit))
  const collectionItems = collections.itemResult?.items || []
  const selectedIsActive = selected?.status !== 'trashed'
  const canEditSelected = canWrite && selectedIsActive
  const renderItem = (item, offset) => {
    ...
    const canMoveDown = total > 0 ? position < total - 1 : (offset < collectionItems.length - 1 || Boolean(collections.itemHasNext))
    return `<article class="account-collection-item">
      <span class="account-collection-item-index" aria-hidden="true">${position + 1}</span>
      <div class="account-collection-item-main">
        <div class="account-collection-item-heading"><h4>${escapeHtml(item.title || '未命名资源')}</h4><span class="account-badge is-muted">${escapeHtml(item.type || 'auto')}</span></div>
        <a class="account-collection-item-url" href="${escapeHtml(item.url || '#')}" target="_blank" rel="noopener noreferrer">${escapeHtml(item.url || '无地址')}</a>
        ${item.coverUrl ? `<small class="account-collection-item-cover">封面：${escapeHtml(item.coverUrl)}</small>` : ''}
      </div>
      ${canEditSelected ? `<div class="account-collection-item-actions"><button type="button" data-account-action="move-collection-item" data-id="${escapeHtml(selected.id)}" data-item-id="${escapeHtml(item.id)}" data-direction="up" ${canMoveUp ? '' : 'disabled'} aria-label="上移资源" title="上移资源">↑</button><button type="button" data-account-action="move-collection-item" data-id="${escapeHtml(selected.id)}" data-item-id="${escapeHtml(item.id)}" data-direction="down" ${canMoveDown ? '' : 'disabled'} aria-label="下移资源" title="下移资源">↓</button><button type="button" data-account-action="edit-collection-item" data-id="${escapeHtml(selected.id)}" data-item-id="${escapeHtml(item.id)}">编辑</button><button type="button" class="is-danger" data-account-action="delete-collection-item" data-id="${escapeHtml(selected.id)}" data-item-id="${escapeHtml(item.id)}">删除</button></div>` : ''}
    </article>`
  }
  const renderCollection = item => {
    const itemIsActive = item.status !== 'trashed'
    const itemCanWrite = canWrite && itemIsActive
    const isPublic = item.isPublic === true || item.public === true || item.visibility === 'public'
    const statusLabel = itemIsActive ? '使用中' : '回收站'
    return `<article class="account-card account-collection-card ${itemIsActive ? '' : 'is-trashed'}">
      <header class="account-collection-card-heading">
        <div class="account-collection-card-title"><span class="account-collection-mark" aria-hidden="true">◎</span><div><div class="account-collection-badges"><span class="account-badge ${itemIsActive ? '' : 'is-muted'}">${statusLabel}</span><span class="account-badge ${isPublic ? '' : 'is-muted'}">${isPublic ? '公开' : '私有'}</span></div><h3>${escapeHtml(item.name || '未命名集合')}</h3></div></div>
        <p>${escapeHtml(item.description || '暂无描述')}</p>
      </header>
      <dl class="account-collection-stats"><div><dt>资源</dt><dd>${Number(item.itemCount || item.resourceCount || 0).toLocaleString()} 项</dd></div><div><dt>占用</dt><dd>${formatBytes(item.byteSize)}</dd></div><div><dt>引用</dt><dd>${Number(item.referenceCount || 0).toLocaleString()}</dd></div><div><dt>更新</dt><dd>${formatDateTime(item.updatedAt)}</dd></div></dl>
      <footer class="account-collection-card-actions"><button type="button" class="account-primary-button" data-account-action="open-collection" data-id="${escapeHtml(item.id)}">打开管理</button><button type="button" data-account-action="collection-references" data-id="${escapeHtml(item.id)}">查看引用</button>${itemCanWrite ? `<button type="button" data-account-action="edit-collection" data-id="${escapeHtml(item.id)}">编辑信息</button>` : ''}${!itemIsActive && canWrite ? `<button type="button" data-account-action="restore-collection" data-id="${escapeHtml(item.id)}">恢复</button><button type="button" class="is-danger" data-account-action="permanent-delete-collection" data-id="${escapeHtml(item.id)}">永久删除</button>` : itemIsActive && canWrite ? `<button type="button" class="is-danger" data-account-action="trash-collection" data-id="${escapeHtml(item.id)}">移入回收站</button>` : ''}</footer>
    </article>`
  }
  const batchOpen = collections.batchText ? ' open' : ''
  const selectedView = selected ? `<section class="account-collection-editor" aria-labelledby="account-collection-editor-title">
    <header class="account-collection-editor-heading">
      <div class="account-collection-editor-copy">
        <button type="button" class="account-collection-back" data-account-action="close-collection">← 返回资源集合</button>
        <div class="account-collection-title-line"><h3 id="account-collection-editor-title">${escapeHtml(selected.name || '资源集合')}</h3><span class="account-badge ${selectedIsActive ? '' : 'is-muted'}">${selectedIsActive ? '使用中' : '回收站'}</span><span class="account-badge ${selected.isPublic || selected.public || selected.visibility === 'public' ? '' : 'is-muted'}">${selected.isPublic || selected.public || selected.visibility === 'public' ? '公开' : '私有'}</span></div>
        <p>${escapeHtml(selected.description || '暂无描述')}</p>
      </div>
      <div class="account-collection-editor-actions"><button type="button" data-account-action="collection-references" data-id="${escapeHtml(selected.id)}">查看引用</button>${canEditSelected ? `<button type="button" data-account-action="edit-collection" data-id="${escapeHtml(selected.id)}">编辑信息</button>` : ''}${!selectedIsActive && canWrite ? `<button type="button" data-account-action="restore-collection" data-id="${escapeHtml(selected.id)}">恢复集合</button>` : ''}</div>
    </header>
    <dl class="account-collection-detail-stats" aria-label="集合概况"><div><dt>资源项</dt><dd>${Number(selected.itemCount || 0).toLocaleString()}</dd></div><div><dt>占用空间</dt><dd>${formatBytes(selected.byteSize)}</dd></div><div><dt>引用点位</dt><dd>${Number(selected.referenceCount || 0).toLocaleString()}</dd></div><div><dt>最后更新</dt><dd>${formatDateTime(selected.updatedAt)}</dd></div></dl>
    ${!selectedIsActive ? `<div class="account-collection-readonly-note">该集合位于回收站，当前只能查看资源项。</div>` : ''}
    <section class="account-collection-items-section" aria-labelledby="account-collection-items-title">
      <header class="account-collection-items-heading">
        <div class="account-collection-items-heading-copy"><span class="account-collection-section-kicker">内容管理</span><div><h4 id="account-collection-items-title">资源项</h4><span>${itemTotal === null ? '数量未知' : `共 ${itemTotal.toLocaleString()} 项`}</span></div></div>
        ${canEditSelected ? `<div class="account-collection-items-actions"><button type="button" class="account-primary-button" data-account-action="add-collection-item" data-id="${escapeHtml(selected.id)}">添加资源</button><details class="account-collection-batch"${batchOpen}><summary><span>批量添加</span><small>一次导入多个地址</small></summary><div class="account-collection-batch-body"><label for="account-collection-batch-input">资源地址</label><textarea id="account-collection-batch-input" rows="3" data-collection-batch placeholder="每行一个 HTTPS 地址">${escapeHtml(collections.batchText || '')}</textarea><small>重复地址会自动跳过。</small><div class="account-collection-tool-actions"><button type="button" class="account-secondary-button" data-account-action="batch-add-collection-items" data-id="${escapeHtml(selected.id)}">开始添加</button></div></div></details></div>` : ''}
      </header>
      <div class="account-collection-item-list">${collectionItems.length ? collectionItems.map(renderItem).join('') : `<div class="account-empty is-compact"><strong>暂无资源项</strong><p>${canEditSelected ? '添加一个资源地址，开始维护这个集合。' : '该集合当前没有资源项。'}</p>${canEditSelected ? `<button type="button" class="account-primary-button" data-account-action="add-collection-item" data-id="${escapeHtml(selected.id)}">添加第一项</button>` : ''}</div>`}</div>
      ${collections.itemResult ? `<nav class="account-collection-pagination" aria-label="资源项分页"><button type="button" data-account-action="collection-item-page" data-id="${escapeHtml(selected.id)}" data-page="${Math.max(1, itemPage - 1)}" ${itemPage <= 1 || state.busy ? 'disabled' : ''}>上一页</button><span>${itemTotal !== null ? `第 ${itemPage} 页，共 ${itemTotalPages} 页 · 共 ${itemTotal.toLocaleString()} 项` : `第 ${itemPage} 页 · 数量未知`}</span><button type="button" data-account-action="collection-item-page" data-id="${escapeHtml(selected.id)}" data-page="${itemPage + 1}" ${!collections.itemHasNext || state.busy ? 'disabled' : ''}>下一页</button></nav>` : ''}
    </section>
  </section>` : ''
  const listView = collections.items.length ? `<div class="account-card-list account-collection-card-list">${collections.items.map(renderCollection).join('')}</div><div class="account-card-actions account-collection-pagination"><button type="button" data-account-action="collection-page" data-page="${Math.max(1, collections.page - 1)}" ${collections.page <= 1 ? 'disabled' : ''}>上一页</button><span>第 ${collections.page} 页，共 ${totalPages} 页</span><button type="button" data-account-action="collection-page" data-page="${collections.page + 1}" ${collections.page >= totalPages ? 'disabled' : ''}>下一页</button></div>` : '<div class="account-empty"><strong>暂无资源集合</strong><p>创建集合后即可在 KML 点位中按 ID 引用。</p></div>'
  const pageHeading = selected ? '' : `<div class="account-section-heading"><div><p class="account-eyebrow">个人数据</p><h2>资源集合</h2><p>独立维护可被多个 KML 点位引用的资源，集合内容按需加载。</p></div>${canWrite ? '<button type="button" class="account-primary-button" data-account-action="create-collection">新建集合</button>' : ''}</div>`
  const filter = selected ? '' : `<form data-account-form="collection-filter" class="account-search-form account-collection-filter"><input name="search" value="${escapeHtml(collections.search || '')}" placeholder="搜索集合名称或描述"><select name="status"><option value="active" ${collections.status !== 'trashed' ? 'selected' : ''}>使用中</option><option value="trashed" ${collections.status === 'trashed' ? 'selected' : ''}>回收站</option></select><select name="visibility"><option value="all" ${(collections.visibility || 'all') === 'all' ? 'selected' : ''}>全部公开状态</option><option value="public" ${collections.visibility === 'public' ? 'selected' : ''}>公开</option><option value="private" ${collections.visibility === 'private' ? 'selected' : ''}>私有</option></select><select name="sort"><option value="updatedAt" ${collections.sort === 'updatedAt' ? 'selected' : ''}>最近更新</option><option value="name" ${collections.sort === 'name' ? 'selected' : ''}>名称</option><option value="itemCount" ${collections.sort === 'itemCount' ? 'selected' : ''}>资源数量</option></select><select name="order"><option value="desc" ${collections.order !== 'asc' ? 'selected' : ''}>降序</option><option value="asc" ${collections.order === 'asc' ? 'selected' : ''}>升序</option></select><button type="submit">查询</button></form>`
  return `<section class="account-panel-section account-collections-panel">${pageHeading}${filter}${selectedView || listView}</section>`
}
