...
`):`// 当前组合图层没有可用图源`;return`
  ...
  `}var Z={root:null,activeTab:`overview`,loading:!1,message:``,error:``,session:null,system:null,cache:null,cacheLoading:!1,cacheError:``,visits:null,visitsLoading:!1,visitsError:``,settings:null,tasks:[],kmls:[],precacheForm:{providerId:``,bounds:{west:113.24,south:23.11,east:113.29,north:23.15},minZoom:12,maxZoom:12,concurrency:4,requestIntervalMs:0,refresh:!1},precacheEstimate:null,precacheEstimateStatus:``,precacheEstimateError:``,expandedTaskIds:new Set,amapLoader:null,AMap:null,map:null,rectangle:null,precacheMapHeight:260},Zn=null;function Qn(e){Zn=e}function Q(e=``,t=``){Z.message=e,Z.error=t,Zn&&Zn(e,t)}function $n(e){Kn(e)&&(Z.activeTab=e)}var er=null;Qn((e,t)=>{er&&=(clearTimeout(er),null);let n=t||e;n&&n!==`正在加载`&&n!==`正在登录`&&(er=setTimeout(()=>{Q(``),document.querySelector(`[data-admin-login]`)?Yn(Z):ir()},4e3))});function tr(){return X(window.location)}function nr(e){window.history.replaceState(null,``,`${Wn(e)}${window.location.hash}`)}function rr(){return Gn(Z.activeTab).render(Z)}function ir(){Xn(Z,rr()),Gn(Z.activeTab).afterRender?.(Z,A)}window.renderDashboard=ir;function ar(e){return{api:A,event:e,renderDashboard:ir,setNotice:Q,showCheckboxConfirm:me,showConfirm:pe,state:Z}}function or(...e){(!e.length||e.includes(Z.activeTab))&&ir()}async function sr(e,t){let n=Gn(Z.activeTab)[e];return n instanceof Function?!!await n(ar(t)):!1}async function cr(e={}){let t=!!e.cacheOnly;Object.assign(Z,{cacheLoading:!0,cacheError:``,visitsLoading:t?Z.visitsLoading:!0,visitsError:t?Z.visitsError:``}),or(`overview`,`cache`),A.cache().then(e=>{Z.cache=e,Z.cacheError=``,e.refreshing&&window.setTimeout(()=>cr({cacheOnly:!0}),1500)}).catch(e=>{Z.cacheError=e.message}).finally(()=>{Z.cacheLoading=!1,or(`cache`)}),!t&&A.visits().then(e=>{Z.visits=e,Z.visitsError=``}).catch(e=>{Z.visitsError=e.message}).finally(()=>{Z.visitsLoading=!1,or(`overview`)})}async function lr(){Z.loading=!0,Q(`正在加载`),ir();try{let[e,t,n,r,i,a,o,s,c,l,u,d,f]=await Promise.all([A.session(),A.system(),A.settings(),A.tasks(),A.kmls(),A.listTileSources(),A.listSourcePresets(),A.listKeyPools(),A.listMapLayers(),A.listProxyOutbounds(),A.listProxyPools(),A.listExternalPublishes(),A.precacheCatalog()]);Object.assign(Z,{session:e,system:t,settings:n,tasks:r,kmls:i,tileSources:a,sourcePresets:o,keyPools:s,mapLayers:c,proxyOutbounds:l,proxyPools:u,externalPublishes:d,precacheCatalog:f,loading:!1}),Q(``),ir(),Gn(Z.activeTab).afterLoad?.(Z,A),cr()}catch(e){Z.loading=!1,e.status===401?(xe(),Q(``,e.message),Yn(Z)):(Q(``,e.message),ir())}}async function ur(e){let t=e.target.closest(`[data-admin-login]`);if(t){e.preventDefault(),Q(`正在登录`),Yn(Z);try{await be({username:t.elements.username.value,password:t.elements.password.value}),Q(``),await lr()}catch(e){Q(``,e.message),Yn(Z)}return}await sr(`handleSubmit`,e)}async function dr(e){let t=e.target.closest(`[data-admin-tab]`);if(t){e.preventDefault(),$n(t.getAttribute(`data-admin-tab`)),nr(Z.activeTab),ir(),Gn(Z.activeTab).afterEnter?.(Z,A);return}let n=e.target.closest(`[data-admin-action]`);if(n){let e=n.getAttribute(`data-admin-action`);if(e===`logout`){xe(),Q(``),Yn(Z);return}if(e===`refresh`){await lr();return}if(e===`close-notice`){Q(``),document.querySelector(`[data-admin-login]`)?Yn(Z):ir();return}}await sr(`handleClick`,e)}async function fr(e){await sr(`handleChange`,e)}async function pr(e={}){if(document.body.classList.add(`admin-view`),$n(tr()),nr(Z.activeTab),Z.amapLoader=e.amapLoader||null,Z.root=document.getElementById(`admin-root`),Z.root.hidden=!1,Z.root.addEventListener(`submit`,ur),Z.root.addEventListener(`click`,dr),Z.root.addEventListener(`change`,fr),Z.root.addEventListener(`input`,fr),!ve()){Yn(Z);return}await lr()}function mr(){`serviceWorker`in navigator&&window.addEventListener(`load`,()=>{navigator.serviceWorker.register(`/sw.js`).catch(e=>{console.warn(`Service Worker 注册失败`,e)})})}var hr=50,gr=[`image`,`video`,`iframe`,`link`],_r={image:`图片`,video:`视频`,iframe:`页面`,link:`链接`},vr=new Set([`jpg`,`jpeg`,`png`,`webp`,`gif`,`avif`]),yr=new Set([`mp4`,`webm`,`mov`,`m4v`,`m3u8`]),br=new Set([`token`,`access_token`,`key`,`api_key`,`apikey`,`secret`,`password`,`signature`,`sign`,`session`,`tk`,`appid`]),xr=null;function $(e){return String(e??``).replaceAll(`&`,`&amp;`).replaceAll(`<`,`&lt;`).replaceAll(`>`,`&gt;`).replaceAll(`"`,`&quot;`).replaceAll(`'`,`&#39;`)}function Sr(e){let t=String(e||``).trim();for(;/^[<([{"'“‘]+/.test(t);)t=t.slice(1);for(;/[>),.;:!?，。；：！？、\]}"'”’]+$/.test(t);)t=t.slice(0,-1);return t.trim()}function Cr(e){try{return new URL(Sr(e))}catch{return null}}function wr(e){let t=e.split(`.`).map(e=>Number(e));if(t.length!==4||t.some(e=>!Number.isInteger(e)||e<0||e>255))return!1;let[n,r]=t;return n===10||n===127||n===169&&r===254||n===172&&r>=16&&r<=31||n===192&&r===168||n===0}function Tr(e){let t=String(e||``).toLowerCase().replace(/^\[|\]$/g,``);return!t||t===`localhost`||t.endsWith(`.localhost`)||t.endsWith(`.local`)||t===`metadata.google.internal`||t===`169.254.169.254`||t===`::1`||t.startsWith(`fc`)||t.startsWith(`fd`)||t.startsWith(`fe80:`)||wr(t)}function Er(e){let t=new URL(e.toString());for(let e of[...t.searchParams.keys()])br.has(e.toLowerCase())&&t.searchParams.set(e,`****`);return t.toString()}function Dr(e){let t=/\.([a-z0-9]+)$/i.exec(e.pathname);return t?t[1].toLowerCase():``}function Or(){let e=window.MAP_SERVICE_KML_IFRAME_ALLOWLIST||window.mapServiceKmlIframeAllowlist||[];return Array.isArray(e)?e:String(e||``).split(`,`).map(e=>e.trim()).filter(Boolean)}function kr(e){let t=e.hostname.toLowerCase(),n=`${e.origin}${e.pathname}`.toLowerCase();return Or().some(e=>{let r=String(e||``).trim().toLowerCase();if(!r)return!1;if(r.startsWith(`https://`))return n.startsWith(r);if(r.startsWith(`*.`)){let e=r.slice(1);return t.endsWith(e)&&t!==e.slice(1)}return t===r})}function Ar(e,t=hr){let n=String(e||``).match(/https?:\/\/[^\s<>"'`]+/gi)||[],r=new Set,i=[],a=!1;for(let e of n){let n=Cr(e);if(!n)continue;let o=n.toString();if(!r.has(o)){if(r.add(o),i.length>=t){a=!0;continue}i.push(n)}}return{urls:i,truncated:a}}function jr(e,t){if(e.protocol!==`https:`||Tr(e.hostname))return null;let n=Dr(e),r=`link`;vr.has(n)?r=`image`:yr.has(n)?r=`video`:kr(e)&&(r=`iframe`);let i=Er(e);return{id:`description-link-${t+1}`,type:r,title:e.hostname,description:``,url:i,displayUrl:i,thumbnailUrl:r===`image`?i:``,sourceType:`description-link`,embedPolicy:r===`iframe`?{sandbox:`allow-scripts allow-forms allow-popups`,referrerPolicy:`no-referrer`}:null}}function Mr(){return gr.map(e=>({type:e,title:_r[e],items:[]}))}function Nr(e){let t=Mr(),n=new Map(t.map(e=>[e.type,e])),{urls:r,truncated:i}=Ar(e?.description||``);r.forEach((e,t)=>{let r=jr(e,t);r&&n.get(r.type)?.items.push(r)});let a={imageCount:n.get(`image`).items.length,videoCount:n.get(`video`).items.length,iframeCount:n.get(`iframe`).items.length,linkCount:n.get(`link`).items.length};return a.hasRichContent=a.imageCount>0||a.videoCount>0||a.iframeCount>0||a.linkCount>0,{featureId:String(e?.id||``),groups:t,contentSummary:a,sourceSummary:{bindings:0,libraries:0,descriptionLinks:r.length,rejected:0,truncated:i}}}function Pr(e){return Fr(Nr(e).contentSummary)}function Fr(e={}){let t=[];return e.imageCount&&t.push(`${e.imageCount} 张图片`),e.videoCount&&t.push(`${e.videoCount} 个视频`),e.iframeCount&&t.push(`${e.iframeCount} 个页面`),e.linkCount&&t.push(`${e.linkCount} 个链接`),t.join(` / `)}function Ir(){let e=document.getElementById(`kml-feature-content-panel`);return e||(e=document.createElement(`section`),e.id=`kml-feature-content-panel`,e.className=`kml-content-panel`,e.setAttribute(`role`,`dialog`),e.setAttribute(`aria-modal`,`false`),e.hidden=!0,document.body.appendChild(e)),e}function Lr(){xr?.abort(),xr=null;let e=document.getElementById(`kml-feature-content-panel`);e&&(e.hidden=!0,e.innerHTML=``)}async function Rr(e,t){if(e?.isPublic&&e.id&&t?.id){xr?.abort(),xr=new AbortController;try{let n=await window.fetch(`/api/v1/kml/shared/${encodeURIComponent(e.id)}/features/${encodeURIComponent(t.id)}/content`,{signal:xr.signal}),r=await n.json();if(!n.ok||r.code!==0)throw Error(r.error?.message||`点位内容加载失败`);return r.result}finally{xr=null}}return Nr(t)}function zr(e,t){let n=String(t?.description||``).trim(),r=Array.isArray(t?.coordinates)&&t.type===`Point`?`${Number(t.coordinates[0]).toFixed(6)}, ${Number(t.coordinates[1]).toFixed(6)}`:``;return`
  ...
  `}function Hr(e){let t=e.embedPolicy||{};return`
    <article class="kml-content-card">
  ...
  `}function Wr(e){if(!e.items?.length)return``;let t={image:Br,video:Vr,iframe:Hr,link:Ur}[e.type]||Ur;return`
    <section class="kml-content-group kml-content-group-${$(e.type)}">
      <h3>${$(e.title||_r[e.type]||`内容`)}</h3>
      <div class="kml-content-items">
        ${e.items.map(t).join(``)}
      </div>
    </section>
  `}function Gr(e,t,n,r,i=``){let a=Fr(r?.contentSummary),o=(r?.groups||[]).map(Wr).join(``);e.innerHTML=`
  ...
  `,e.querySelector(`[data-kml-content-close]`)?.addEventListener(`click`,Lr)}async function Kr(e,t){let n=Ir();n.hidden=!1,n.innerHTML=`
    <header class="kml-content-header">
      <div>
  ...
  `,n.querySelector(`[data-kml-content-close]`)?.addEventListener(`click`,Lr);try{Gr(n,e,t,await Rr(e,t))}catch(r){if(r.name===`AbortError`)return;Gr(n,e,t,Nr(t),r.message||`点位内容加载失败，已展示本地解析结果。`)}}async function qr(e){let{init:t,title:n=`私有地图服务`,message:r=`管理员启用了访问控制，请输入密码解锁`,submitText:i=`载入地图`,loadingText:a=`正在验证...`}=e;try{(await Se()).required?Jr({init:t,title:n,message:r,submitText:i,loadingText:a}):await t()}catch(e){console.error(`Failed to check map access status`,e),Jr({init:t,title:n,message:`访问状态检查失败，请稍后重试`,submitText:i,loadingText:a,allowRetry:!0})}}function Jr(e){let{init:t,title:n,message:r,submitText:i,loadingText:a,allowRetry:o=!1}=e;document.getElementById(`map-lock-screen`)?.remove();let s=document.createElement(`div`);s.id=`map-lock-screen`,s.className=`lock-screen-backdrop`,s.innerHTML=`
  ...
  `,document.body.appendChild(s);let c=document.getElementById(`lock-screen-form`),l=document.getElementById(`lock-screen-error`);s.querySelector(`[data-lock-retry]`)?.addEventListener(`click`,()=>{s.remove(),qr({init:t,title:n,submitText:i,loadingText:a})}),c.addEventListener(`submit`,async e=>{e.preventDefault(),l.style.display=`none`;let n=c.elements.password.value.trim();if(!n)return;let r=c.querySelector(`button[type="submit"]`);try{r.disabled=!0,r.textContent=a,await Ce(n),s.remove(),await t()}catch(e){r.disabled=!1,r.textContent=i,l.textContent=e.message||`访问密码错误`,l.style.display=`block`}})}export{d as C,c as D,l as E,h as S,o as T,x as _,pr as a,p as b,Dt as c,pe as d,he as f,ue as g,le as h,mr as i,j as l,ce as m,Pr as n,qn as o,w as p,Kr as r,kt as s,qr as t,fe as u,se as v,u as w,m as x,_ as y};