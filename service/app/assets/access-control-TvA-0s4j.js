...
`)}var Ft=50,It=[`image`,`video`,`iframe`,`link`],Lt={image:`图片`,video:`视频`,iframe:`页面`,link:`链接`},Rt=new Set([`jpg`,`jpeg`,`png`,`webp`,`gif`,`avif`]),zt=new Set([`mp4`,`webm`,`mov`,`m4v`,`m3u8`]),Bt=new Set([`token`,`access_token`,`key`,`api_key`,`apikey`,`secret`,`password`,`signature`,`sign`,`session`,`tk`,`appid`]),Vt=null;function G(e){return String(e??``).replaceAll(`&`,`&amp;`).replaceAll(`<`,`&lt;`).replaceAll(`>`,`&gt;`).replaceAll(`"`,`&quot;`).replaceAll(`'`,`&#39;`)}function Ht(e){let t=String(e||``).trim();for(;/^[<([{"'“‘]+/.test(t);)t=t.slice(1);for(;/[>),.;:!?，。；：！？、\]}"'”’]+$/.test(t);)t=t.slice(0,-1);return t.trim()}function Ut(e){try{return new URL(Ht(e))}catch{return null}}function Wt(e){let t=e.split(`.`).map(e=>Number(e));if(t.length!==4||t.some(e=>!Number.isInteger(e)||e<0||e>255))return!1;let[n,r]=t;return n===10||n===127||n===169&&r===254||n===172&&r>=16&&r<=31||n===192&&r===168||n===0}function K(e){let t=String(e||``).toLowerCase().replace(/^\[|\]$/g,``);return!t||t===`localhost`||t.endsWith(`.localhost`)||t.endsWith(`.local`)||t===`metadata.google.internal`||t===`169.254.169.254`||t===`::1`||t.startsWith(`fc`)||t.startsWith(`fd`)||t.startsWith(`fe80:`)||Wt(t)}function Gt(e){let t=new URL(e.toString());for(let e of[...t.searchParams.keys()])Bt.has(e.toLowerCase())&&t.searchParams.set(e,`****`);return t.toString()}function q(e){let t=/\.([a-z0-9]+)$/i.exec(e.pathname);return t?t[1].toLowerCase():``}function Kt(){let e=window.MAP_SERVICE_KML_IFRAME_ALLOWLIST||window.mapServiceKmlIframeAllowlist||[];return Array.isArray(e)?e:String(e||``).split(`,`).map(e=>e.trim()).filter(Boolean)}function qt(e){let t=e.hostname.toLowerCase(),n=`${e.origin}${e.pathname}`.toLowerCase();return Kt().some(e=>{let r=String(e||``).trim().toLowerCase();if(!r)return!1;if(r.startsWith(`https://`))return n.startsWith(r);if(r.startsWith(`*.`)){let e=r.slice(1);return t.endsWith(e)&&t!==e.slice(1)}return t===r})}function Jt(e,t=Ft){let n=String(e||``).match(/https?:\/\/[^\s<>"'`]+/gi)||[],r=new Set,i=[],a=!1;for(let e of n){let n=Ut(e);if(!n)continue;let o=n.toString();if(!r.has(o)){if(r.add(o),i.length>=t){a=!0;continue}i.push(n)}}return{urls:i,truncated:a}}function Yt(e,t){if(e.protocol!==`https:`||K(e.hostname))return null;let n=q(e),r=`link`;Rt.has(n)?r=`image`:zt.has(n)?r=`video`:qt(e)&&(r=`iframe`);let i=Gt(e);return{id:`description-link-${t+1}`,type:r,title:e.hostname,description:``,url:i,displayUrl:i,thumbnailUrl:r===`image`?i:``,sourceType:`description-link`,embedPolicy:r===`iframe`?{sandbox:`allow-scripts allow-forms allow-popups`,referrerPolicy:`no-referrer`}:null}}function Xt(){return It.map(e=>({type:e,title:Lt[e],items:[]}))}function Zt(e){let t=Xt(),n=new Map(t.map(e=>[e.type,e])),{urls:r,truncated:i}=Jt(e?.description||``);r.forEach((e,t)=>{let r=Yt(e,t);r&&n.get(r.type)?.items.push(r)});let a={imageCount:n.get(`image`).items.length,videoCount:n.get(`video`).items.length,iframeCount:n.get(`iframe`).items.length,linkCount:n.get(`link`).items.length};return a.hasRichContent=a.imageCount>0||a.videoCount>0||a.iframeCount>0||a.linkCount>0,{featureId:String(e?.id||``),groups:t,contentSummary:a,sourceSummary:{bindings:0,libraries:0,descriptionLinks:r.length,rejected:0,truncated:i}}}function Qt(e){return J(Zt(e).contentSummary)}function J(e={}){let t=[];return e.imageCount&&t.push(`${e.imageCount} 张图片`),e.videoCount&&t.push(`${e.videoCount} 个视频`),e.iframeCount&&t.push(`${e.iframeCount} 个页面`),e.linkCount&&t.push(`${e.linkCount} 个链接`),t.join(` / `)}function $t(){let e=document.getElementById(`kml-feature-content-panel`);return e||(e=document.createElement(`section`),e.id=`kml-feature-content-panel`,e.className=`kml-content-panel`,e.setAttribute(`role`,`dialog`),e.setAttribute(`aria-modal`,`false`),e.hidden=!0,document.body.appendChild(e)),e}function en(){Vt?.abort(),Vt=null;let e=document.getElementById(`kml-feature-content-panel`);e&&(e.hidden=!0,e.innerHTML=``)}async function tn(e,t){if(e?.isPublic&&e.id&&t?.id){Vt?.abort(),Vt=new AbortController;try{let n=await window.fetch(`/api/v1/kml/shared/${encodeURIComponent(e.id)}/features/${encodeURIComponent(t.id)}/content`,{signal:Vt.signal}),r=await n.json();if(!n.ok||r.code!==0)throw Error(r.error?.message||`点位内容加载失败`);return r.result}finally{Vt=null}}return Zt(t)}function nn(e,t){let n=String(t?.description||``).trim(),r=Array.isArray(t?.coordinates)&&t.type===`Point`?`${Number(t.coordinates[0]).toFixed(6)}, ${Number(t.coordinates[1]).toFixed(6)}`:``;return`
  ...
  `}function on(e){let t=e.embedPolicy||{};return`
    <article class="kml-content-card">
  ...
  `}function Y(e){if(!e.items?.length)return``;let t={image:rn,video:an,iframe:on,link:sn}[e.type]||sn;return`
    <section class="kml-content-group kml-content-group-${G(e.type)}">
      <h3>${G(e.title||Lt[e.type]||`内容`)}</h3>
      <div class="kml-content-items">
        ${e.items.map(t).join(``)}
      </div>
    </section>
  `}function cn(e,t,n,r,i=``){let a=J(r?.contentSummary),o=(r?.groups||[]).map(Y).join(``);e.innerHTML=`
  ...
  `,e.querySelector(`[data-kml-content-close]`)?.addEventListener(`click`,en)}async function ln(e,t){let n=$t();n.hidden=!1,n.innerHTML=`
    <header class="kml-content-header">
      <div>
    ...
    <div class="admin-grid">
      <section class="admin-panel">
        ...
        <dl class="admin-metrics">
          ...
          <div><dt>运行</dt><dd>${On(t?.uptime||0)}</dd></div>
          <div><dt>环境</dt><dd>${Q(t?.env||`-`)}</dd></div>
          <div><dt>时间</dt><dd>${kn(t?.serverTime)}</dd></div>
        </dl>
      </section>
  ...
  `}function Mn(e){let t=e.cache||{},n=e.cacheError||(e.cacheLoading?`统计中`:``),r=t.bySource||{},i=e.tileSources||[];return`
    <section class="admin-panel">
      <div class="admin-panel-head">
        <h2>缓存治理</h2>
      ...
      <dl class="admin-metrics admin-metrics-five">
        ...
        <div class="metric-size"><dt>总大小</dt><dd>${Dn(t.bytes||0)}</dd></div>
  ...
  `}async function Nn({api:e,event:t,renderDashboard:n,setNotice:r,showConfirm:i,state:a}){let o=t.target.closest(`[data-admin-action]`);if(!o)return!1;let s=o.getAttribute(`data-admin-action`);if(s===`clear-cache`){if(!await i(`清空所有瓦片缓存？此操作不可逆！`))return!0;try{await e.clearCache(),a.cache=await e.cache(),r(`所有瓦片缓存已清空`),n()}catch(e){r(``,e.message),n()}return!0}if(s===`clear-source-cache`){let t=o.getAttribute(`data-source-id`);if(!t)return!1;let s=(a.tileSources||[]).find(e=>e.id===t),c=s?s.name:t;if(!await i(`确定要清空图源【${c}】的全部瓦片缓存吗？`))return!0;try{await e.clearCache({sourceId:t}),a.cache=await e.cache(),r(`已清空图源【${c}】的专属瓦片缓存`),n()}catch(e){r(``,e.message),n()}return!0}return!1}var Pn={queued:`排队中`,running:`执行中`,pausing:`暂停中`,paused:`已暂停`,completed:`已完成`,completed_with_errors:`完成有错误`,failed:`失败`,interrupted:`已中断`,deleting:`删除中`},Fn=1e3,In=1500,Ln=new Set([`queued`,`running`,`pausing`,`deleting`]),Rn=null,zn=0,Bn=``,Vn=``,Hn=null,Un=0;function Wn(e){return Pn[e]||e}var Gn={queued:`⏳`,running:`⚙`,pausing:`⏸`,paused:`⏸`,completed:`✓`,completed_with_errors:`⚠`,failed:`✗`,interrupted:`⏹`,deleting:`🗑`};function Kn(e){return Gn[e]||`•`}function qn(e,t){let n=t[0],r=e.precacheForm||{};return{providerId:t.some(e=>e.id===r.providerId)?r.providerId:n?.id||``,bounds:{west:Number(r.bounds?.west??113.24),south:Number(r.bounds?.south??23.11),east:Number(r.bounds?.east??113.29),north:Number(r.bounds?.north??23.15)},minZoom:Number(r.minZoom??12),maxZoom:Number(r.maxZoom??12),concurrency:Number(r.concurrency??4),requestIntervalMs:Number(r.requestIntervalMs??0),refresh:!!r.refresh}}function Jn(e,t){return e.find(e=>e.id===t.providerId)||e[0]||null}function Yn(e){let t=e.expandedTaskIds||new Set;return(e.tasks||[]).map(e=>({...e,expanded:t.has(e.id)}))}function Xn(e=[]){return e.some(e=>Ln.has(e.status))}function Zn(e){let t=e.precacheCatalog||[],n=qn(e,t),r=Jn(t,n),i=Yn(e);return`
  ...
  `}function Qn(e){let t=e.precacheEstimate;return e.precacheEstimateStatus===`loading`?`<div class="admin-estimate" data-precache-estimate><p>正在估算瓦片数量和下载体积</p></div>`:e.precacheEstimateError?`<div class="admin-estimate is-error" data-precache-estimate><p>${Q(e.precacheEstimateError)}</p></div>`:t?`
    <div class="admin-estimate ${t.withinLimit?``:`is-warning`}" data-precache-estimate>
      <dl class="admin-metrics">
        <div><dt>预计文件</dt><dd>${Q(t.total||0)}</dd></div>
        <div><dt>估算体积</dt><dd>${Dn(t.estimatedBytesRange?.min||0)} - ${Dn(t.estimatedBytesRange?.max||0)}</dd></div>
        <div><dt>建议上限</dt><dd>${Q(t.maxTiles||0)}</dd></div>
        <div><dt>建议</dt><dd>${t.withinLimit?`可以创建`:`任务较大，建议设置请求间隔`}</dd></div>
      </dl>
      <p>${tr(t)}</p>
    </div>
  `:`<div class="admin-estimate" data-precache-estimate><p>停止移动地图后会自动估算任务规模</p></div>`}function $n(e){let t=e.root?.querySelector(`[data-precache-estimate]`);t&&(t.outerHTML=Qn(e))}function er(e){return e.length?e.map(e=>`Z${e.z}: ${e.count}`).join(`，`):`暂无分级明细`}function tr(e){if(e.targetType===`layer`){let t=e.sources||[];return t.length?t.map(e=>`${e.sourceName||e.sourceId}: ${e.total||0} 张，约 ${Dn(e.estimatedBytes||0)}`).join(`，`):`暂无可预缓存图源明细`}return er(e.ranges||[])}function nr(e){return`
    <section class="admin-panel admin-panel-wide admin-precache-task-panel" data-precache-task-panel>
      ...
      <div class="admin-task-list">
        ${e.slice(0,10).map(e=>rr(e)).join(``)||`<p class="admin-empty">暂无任务</p>`}
      </div>
    </section>
  `}function rr(e){let t=e.expanded;return`
    <article class="admin-task-card">
      <div class="admin-task-main">
        <div class="admin-task-title">
          <span class="admin-status admin-status-${e.status}" title="${Q(Wn(e.status))}">${Q(Kn(e.status))}</span>
          <strong>${Q(e.providerId)}</strong>
          <small>${kn(e.updatedAt)}</small>
        </div>
        ${ir(e)}
      </div>
      <dl class="admin-task-summary">
        <div><dt>体积</dt><dd>${Dn(e.bytes||0)}</dd></div>
  ...
  `}function ir(e){let t=Number(e.completed||0),n=Number(e.total||0),r=n?Math.min(100,Math.round(t/n*100)):0;return`
    <div class="admin-task-progress">
      <span>${Q(t)} / ${Q(n)} (${r}%)</span>
      <small>成功 ${Q(e.succeeded||0)}，失败 ${Q(e.failed||0)}</small>
    </div>
  `}function ar(e){let t=[`queued`,`running`].includes(e.status),n=[`paused`,`interrupted`,`failed`,`completed_with_errors`].includes(e.status),r=e.status!==`deleting`,i=e.expanded?`收起`:`详情`;return`
  ...
  `}function or(e){let t=e.bounds||{},n=e.ranges||[],r=e.errors||[],i=Object.values(e.failedTiles||{});return`
    <div class="admin-task-details">
      <dl>
        ...
        <div><dt>创建时间</dt><dd>${kn(e.createdAt)}</dd></div>
        <div><dt>完成时间</dt><dd>${kn(e.finishedAt)}</dd></div>
      </dl>
      ${i.length?`
  ...
  `}function sr(e,t){Tr(e,t);let n=t.elements.providerId,r=n.options[n.selectedIndex]?.getAttribute(`data-type`)||`source`,i=n.value;return{targetType:r,targetId:i,providerId:i,bounds:{west:Number(t.elements.west.value),south:Number(t.elements.south.value),east:Number(t.elements.east.value),north:Number(t.elements.north.value)},minZoom:Number(t.elements.minZoom.value),maxZoom:Number(t.elements.maxZoom.value),concurrency:Number(t.elements.concurrency.value),requestIntervalMs:Number(t.elements.requestIntervalMs.value),refresh:t.elements.refresh.checked}}function cr(e,t){e.tasks=e.tasks.map(e=>e.id===t.id?t:e)}function lr(e,t){e.tasks=e.tasks.filter(e=>e.id!==t)}function ur(e){let t=e.root?.querySelector(`[data-precache-task-panel]`);t&&(t.outerHTML=nr(Yn(e)))}function dr(e){return Array.isArray(e)?`[${e.map(dr).join(`,`)}]`:e&&typeof e==`object`?`{${Object.keys(e).sort().map(t=>`${JSON.stringify(t)}:${dr(e[t])}`).join(`,`)}}`:JSON.stringify(e)}function fr(e){zn+=1,Bn=``,e.precacheEstimate=null,e.precacheEstimateStatus=``,e.precacheEstimateError=``,$n(e)}function pr(e,t,n=Fn){e.activeTab===`precache`&&(window.clearTimeout(Rn),Rn=window.setTimeout(()=>{mr(e,t)},n))}async function mr(e,t){if(e.activeTab!==`precache`)return;let n=e.root?.querySelector(`[data-precache-form]`);if(!n)return;let r=sr(e,n),i=dr(r);if(i===Vn||i===Bn)return;let a=zn+1;zn=a,Bn=i,e.precacheEstimateStatus=`loading`,e.precacheEstimateError=``,$n(e);try{let n=await t.estimateTask(r);if(a!==zn)return;e.precacheEstimate=n,e.precacheEstimateStatus=``,e.precacheEstimateError=``,Vn=i,Bn=``,$n(e)}catch(t){if(a!==zn)return;e.precacheEstimate=null,e.precacheEstimateStatus=``,e.precacheEstimateError=t.message,Bn=``,$n(e)}}function hr(e,t,n=In){window.clearTimeout(Hn),!(e.activeTab!==`precache`||!Xn(e.tasks))&&(Hn=window.setTimeout(()=>{gr(e,t)},n))}async function gr(e,t){if(e.activeTab!==`precache`)return;let n=Un+1;Un=n;try{let r=await t.tasks();if(n!==Un||e.activeTab!==`precache`)return;e.tasks=r,ur(e),hr(e,t)}catch(n){console.warn(`预缓存任务状态刷新失败`,n),hr(e,t,In*2)}}async function _r({api:e,event:t,renderDashboard:n,setNotice:r,state:i}){let a=t.target.closest(`[data-precache-form]`),o=t.target.closest(`[data-place-search-form]`);if(a)return t.preventDefault(),e.createTask(sr(i,a)).then(t=>{Array.isArray(t)?(i.tasks=[...t,...i.tasks],r(`成功创建了 ${t.length} 个图源缓存子任务`)):(i.tasks=[t,...i.tasks],r(`预缓存任务已创建`)),n(),hr(i,e,300)}).catch(e=>{r(``,e.message),n()}),!0;if(o){t.preventDefault();let e=o.elements.keyword.value.trim();return e&&await Ar(i,e),!0}return!1}async function vr({api:e,event:t,renderDashboard:n,setNotice:r,showCheckboxConfirm:i,showConfirm:a,state:o}){let s=t.target.closest(`[data-precache-task-action]`);if(s)return await br({actionTarget:s,api:e,renderDashboard:n,setNotice:r,showCheckboxConfirm:i,showConfirm:a,state:o}),!0;let c=t.target.closest(`[data-place-lng][data-place-lat]`);return c?(Or(o,Number(c.getAttribute(`data-place-lng`)),Number(c.getAttribute(`data-place-lat`))),!0):t.target.closest(`[data-admin-action]`)?.getAttribute(`data-admin-action`)===`sync-bounds`?(wr(o),fr(o),pr(o,e),!0):!1}async function yr({api:e,event:t,state:n}){let r=t.target.closest(`[data-precache-form]`);return r?(Tr(n,r),fr(n),pr(n,e),!0):!1}async function br({actionTarget:e,api:t,renderDashboard:n,setNotice:r,showCheckboxConfirm:i,showConfirm:a,state:o}){let s=e.getAttribute(`data-precache-task-action`),c=e.getAttribute(`data-task-id`),l=o.tasks.find(e=>e.id===c);if(!(!s||!c))try{if(s===`pause`){cr(o,await t.pauseTask(c)),r(`预缓存任务已暂停`),n(),hr(o,t,300);return}if(s===`resume`){cr(o,await t.resumeTask(c)),r(`预缓存任务已继续`),n(),hr(o,t,300);return}if(s===`delete`){let e=!1;if(i instanceof Function){let t=await i(`删除此预缓存任务？执行中的任务会停止并从列表移除。`,{confirmText:`删除`,checkboxLabel:`同时删除该任务范围内已缓存的瓦片文件`});if(!t?.confirmed)return;e=!!t.checked}else if(!await a(`删除此预缓存任务？执行中的任务会停止并从列表移除。`))return;await t.deleteTask(c,{deleteCache:e}),lr(o,c),r(e?`预缓存任务已删除，关联缓存文件清理已提交`:`预缓存任务已删除`),n();return}if(s===`edit`&&l){Dr(o,l),fr(o),pr(o,t),r(`任务参数已回填，可调整后创建新任务`),n();return}if(s===`update`&&l){o.tasks=[await t.createTask({providerId:l.providerId,bounds:l.bounds,minZoom:l.minZoom,maxZoom:l.maxZoom,concurrency:l.concurrency,requestIntervalMs:l.requestIntervalMs||0,refresh:!1}),...o.tasks],r(`更新任务已创建，将跳过新鲜缓存并补齐缺失瓦片`),n(),hr(o,t,300);return}if(s===`toggle-details`){o.expandedTaskIds.has(c)?o.expandedTaskIds.delete(c):o.expandedTaskIds.add(c),n();return}s===`preview`&&l&&(window.location.href=Er(l))}catch(e){r(``,e.message),n(),hr(o,t,300)}}async function xr(e){return e.AMap?e.AMap:(window._AMapSecurityConfig={securityJsCode:m.securityJsCode},e.amapLoader?(e.AMap=await e.amapLoader.load({key:m.key,version:`2.0`,plugins:m.plugins}).catch(e=>(console.warn(`高德 JSAPI 加载失败，后台地点搜索不可用`,e),null)),e.AMap):(console.warn(`高德 JSAPI Loader 未初始化，后台地点搜索不可用`),null))}function Sr(e){let t=e.root.querySelector(`[data-precache-form]`);return{west:Number(t?.elements.west.value),south:Number(t?.elements.south.value),east:Number(t?.elements.east.value),north:Number(t?.elements.north.value)}}function Cr(e,t){let n=e.root.querySelector(`[data-precache-form]`);n&&(n.elements.west.value=t.getWest().toFixed(6),n.elements.south.value=t.getSouth().toFixed(6),n.elements.east.value=t.getEast().toFixed(6),n.elements.north.value=t.getNorth().toFixed(6),Tr(e,n))}function wr(e){if(!e.map||!e.rectangle)return;let t=e.map.getBounds();e.rectangle.setBounds(t),Cr(e,t)}function Tr(e,t){return t?(e.precacheForm={providerId:t.elements.providerId.value,bounds:{west:Number(t.elements.west.value),south:Number(t.elements.south.value),east:Number(t.elements.east.value),north:Number(t.elements.north.value)},minZoom:Number(t.elements.minZoom.value),maxZoom:Number(t.elements.maxZoom.value),concurrency:Number(t.elements.concurrency.value),requestIntervalMs:Number(t.elements.requestIntervalMs.value),refresh:t.elements.refresh.checked},e.precacheForm):null}function Er(e){let t=e.bounds||{},n=(Number(t.south)+Number(t.north))/2,r=(Number(t.west)+Number(t.east))/2,i=Number(e.maxZoom||e.minZoom||h.zoom);if(!Number.isFinite(n)||!Number.isFinite(r))return`/`;let a=[n.toFixed(6),r.toFixed(6),Math.max(3,Math.min(20,i)),0].join(`,`);return`/?coords=${encodeURIComponent(a)}`}function Dr(e,t){if(!t)return;let n=t.bounds||{};e.precacheForm={providerId:t.providerId||e.precacheForm?.providerId||``,bounds:{west:Number(n.west),south:Number(n.south),east:Number(n.east),north:Number(n.north)},minZoom:Number(t.minZoom),maxZoom:Number(t.maxZoom),concurrency:Number(t.concurrency||e.precacheForm?.concurrency||4),requestIntervalMs:Number(t.requestIntervalMs||e.precacheForm?.requestIntervalMs||0),refresh:!1}}function Or(e,t,n,r=15){e.map&&(e.map.setView([n,t],r),wr(e),e.onPrecacheBoundsChange instanceof Function&&e.onPrecacheBoundsChange())}function kr(e,t){hr(e,t);let n=e.root.querySelector(`#admin-precache-map`);if(!n)return;e.precacheMapHeight&&(n.style.height=`${e.precacheMapHeight}px`),e.onPrecacheBoundsChange=()=>pr(e,t),document.querySelectorAll(`.amap-sug-result`).forEach(e=>e.remove()),e.map&&(e.map.remove(),e.map=null,e.rectangle=null);let r=p.default.map(n,{center:h.center,zoom:Math.min(h.zoom,13),zoomControl:!0,attributionControl:!1});p.default.tileLayer(An(g,`https://webst01.is.autonavi.com/appmaptile?style=8&x={x}&y={y}&z={z}`),{minZoom:3,maxZoom:18,keepBuffer:4}).addTo(r);let i=Sr(e),a=p.default.latLngBounds([i.south,i.west],[i.north,i.east]);e.rectangle=p.default.rectangle(a,{color:`#0f766e`,weight:2,fillColor:`#f59e0b`,fillOpacity:.14}).addTo(r);let o=!1;r.fitBounds(a),r.on(`moveend zoomend`,()=>wr(e)),r.on(`moveend zoomend`,()=>{o&&e.onPrecacheBoundsChange instanceof Function&&e.onPrecacheBoundsChange()}),window.requestAnimationFrame(()=>{o=!0}),e.map=r;let s=e.root.querySelector(`[data-place-search-form] input[name="keyword"]`);s&&(s.id=`admin-precache-search-input`,xr(e).then(t=>{t&&t.AutoComplete&&new t.AutoComplete({input:`admin-precache-search-input`}).on(`select`,t=>{if(t.poi?.location){let{lng:n,lat:r}=t.poi.location;Or(e,n,r)}})}));let c=e.root.querySelector(`.admin-map-resizer`);if(c&&n){let t=0,r=0,i=e=>{t=e.clientY,r=n.offsetHeight,document.addEventListener(`pointermove`,a),document.addEventListener(`pointerup`,o),c.classList.add(`is-dragging`),e.preventDefault()},a=i=>{let a=i.clientY-t,o=Math.min(800,Math.max(150,r+a));n.style.height=`${o}px`,e.precacheMapHeight=o,e.map&&e.map.invalidateSize()},o=()=>{document.removeEventListener(`pointermove`,a),document.removeEventListener(`pointerup`,o),c.classList.remove(`is-dragging`)};c.addEventListener(`pointerdown`,i)}}async function Ar(e,t){let n=e.root.querySelector(`[data-place-search-results]`);if(!n)return;let r=await xr(e);if(!r){n.innerHTML=`<p>高德搜索暂不可用</p>`;return}let i=new r.PlaceSearch({pageSize:8,pageIndex:1});n.innerHTML=`<p>正在搜索</p>`,i.search(t,(e,t)=>{if(e!==`complete`||!t?.poiList?.pois?.length){n.innerHTML=`<p>没有找到匹配地点</p>`;return}n.innerHTML=t.poiList.pois.map(e=>{let t=e.location;return`
        <button type="button" data-place-lng="${t.lng}" data-place-lat="${t.lat}">
          <strong>${Q(e.name)}</strong>
          <span>${Q(e.address||e.district||``)}</span>
        </button>
      `}).join(``)})}var jr=4;function Mr(e){let t=e.settings?.access||{},n=e.settings?.auth||{},r=n.tokenTtl?Math.round(n.tokenTtl/(1e3*60*60*24)):15;return`
    <div class="admin-grid">
      <section class="admin-panel">
        <div class="admin-panel-head">
          <h2>访问控制</h2>
        ...
        <form class="admin-form" data-access-form autocomplete="off">
          ...
          <label>
            <span>设置访问密码</span>
            <input name="accessPassword" type="password" autocomplete="new-password" placeholder="${t.hasPassword?`已设置，输入新密码以修改`:`输入至少 ${jr} 位访问密码`}">
          </label>
          ${t.hasPassword?`
            <label class="admin-check">
  ...
  `}async function Fr({api:e,event:t,renderDashboard:n,setNotice:r,showConfirm:i,state:a}){let o=t.target.closest(`[data-admin-action]`);if(!o)return!1;let s=o.getAttribute(`data-admin-action`),c=o.getAttribute(`data-kml-id`);if(s===`create-blank-kml`){let t=await Ce({title:`新建公共 KML`,fields:[{name:`name`,label:`图层名称`,type:`text`}],values:{name:`新建公共 KML ${a.kmls.length+1}`}});if(!t||!t.name?.trim())return!0;try{r(`正在创建...`),await e.createKml({name:t.name.trim(),status:`draft`,coordCorrection:`wgs84-to-gcj02`,features:[]}),a.kmls=await e.kmls(),r(`新建成功`),n()}catch(e){r(``,e.message),n()}return!0}if(s===`trigger-import`)return document.getElementById(`admin-kml-file-input`)?.click(),!0;if(s===`set-status`){let t=o.getAttribute(`data-kml-status`);try{r(`正在更新状态...`),await e.updateKml(c,{status:t}),a.kmls=await e.kmls(),r(`状态已更新`),n()}catch(e){r(``,e.message),n()}return!0}if(s===`rename`){let t=o.getAttribute(`data-kml-name`),i=await Ce({title:`重命名公共 KML`,fields:[{name:`name`,label:`图层名称`,type:`text`}],values:{name:t}});if(!i||!i.name?.trim()||i.name.trim()===t)return!0;try{r(`正在重命名...`),await e.updateKml(c,{name:i.name.trim()}),a.kmls=await e.kmls(),r(`重命名成功`),n()}catch(e){r(``,e.message),n()}return!0}if(s===`export`){try{r(`正在获取数据...`);let t=await e.getKml(c);r(``);let n=Pt(t.name,t.features||[]),i=new Blob([n],{type:`application/vnd.google-earth.kml+xml;charset=utf-8`}),a=URL.createObjectURL(i),o=document.createElement(`a`);o.href=a,o.download=`${t.name}.kml`,document.body.appendChild(o),o.click(),document.body.removeChild(o),URL.revokeObjectURL(a)}catch(e){r(``,e.message),n()}return!0}if(s===`delete`){if(!await i(`确认永久删除此公共 KML 图层及其中所有要素？此操作不可撤销。`))return!0;try{r(`正在删除...`),await e.deleteKml(c),a.kmls=await e.kmls(),r(`删除成功`),n()}catch(e){r(``,e.message),n()}return!0}return!1}async function Ir({api:e,event:t,renderDashboard:n,setNotice:r,state:i}){let a=t.target;if(a.id===`admin-kml-file-input`){let t=a.files[0];if(!t)return!0;let o=new FormData;o.append(`file`,t),o.append(`name`,t.name),o.append(`status`,`draft`),o.append(`coordCorrection`,`wgs84-to-gcj02`);try{r(`正在导入 KML 文件...`),await e.importKml(o),i.kmls=await e.kmls(),r(`导入成功（默认为草稿状态）`),n()}catch(e){r(``,e.message),n()}finally{a.value=``}return!0}if(a.matches(`[data-admin-action="toggle-correction"]`)){let t=a.getAttribute(`data-kml-id`),o=a.checked?`wgs84-to-gcj02`:`none`;try{r(`正在更新纠偏设置...`),await e.updateKml(t,{coordCorrection:o}),i.kmls=await e.kmls(),r(`纠偏设置已更新`),n()}catch(e){r(``,e.message),n()}return!0}return!1}var Lr=216e5,Rr=2592e6,zr=60*1e3,Br=60*zr,Vr=24*Br,Hr={1:256,2:512,3:768},Ur=[[`xyz`,`XYZ 栅格`],[`tms`,`TMS 栅格`],[`xyz-raster`,`XYZ 栅格`],[`tms-raster`,`TMS 栅格`],[`wmts-raster`,`WMTS 栅格`],[`arcgis-raster`,`ArcGIS 栅格`],[`quadkey-raster`,`QuadKey 栅格`],[`time-raster`,`时间序列栅格`],[`mvt`,`MVT 矢量瓦片`],[`vector-tilejson`,`矢量 TileJSON`],[`vector-style`,`矢量 Style JSON`],[`pmtiles-vector`,`PMTiles 矢量`],[`pmtiles-raster`,`PMTiles 栅格`],[`google-map-tiles-api`,`Google Map Tiles API`]],Wr=[[`satellite`,`卫星`],[`road`,`街道/道路`],[`street`,`街道/道路`],[`label`,`注记`],[`terrain`,`地形`],[`vector`,`矢量`],[`overlay`,`叠加`],[`weather`,`天气`],[`other`,`其他`],[`custom`,`自定义`]],Gr=[[`query`,`Query 参数`],[`header`,`请求头`],[`bearer`,`Bearer Token`],[`session`,`会话/适配器`]],Kr=[[`round_robin`,`健康 Key 轮询`],[`priority_failover`,`优先级失败切换`],[`random`,`健康 Key 随机`],[`weighted_round_robin`,`按权重轮询`]],qr=[[`global`,`全局`],[`source`,`图源`],[`publish`,`发布项`]],Jr=[[`leaflet`,`Leaflet 栅格`],[`maplibre`,`MapLibre 矢量`],[`cesium`,`Cesium 3D`]],Yr=new Set([`mvt`,`vector-tilejson`,`vector-style`,`pmtiles-vector`]),Xr=new Set([`pmtiles-vector`,`pmtiles-raster`]);function Zr(e){return e?.errorMessage||e?.error||``}function Qr(e,t=``){return e.map(([e,n])=>`<option value="${Q(e)}" ${t===e?`selected`:``}>${Q(n)}</option>`).join(``)}function $r(e,t){return!e||t.some(([t])=>t===e)?``:`<option value="${Q(e)}" selected>${Q(e)}</option>`}function ei(e=``){return Ur.find(([t])=>t===e)?.[1]||e||`-`}function ti(e=``){return Yr.has(e)}function ni(e=``){return Xr.has(e)}function ri(e={},t){return e.entry?.[t]||e[t]||``}function ii(e={}){return e.kind===`vector-style`?ri(e,`styleJsonUrl`):e.kind===`vector-tilejson`?ri(e,`tileJsonUrl`):ni(e.kind)?ri(e,`pmtilesUrl`):ri(e,`template`)}function ai(e,t=`是`,n=`否`){return e?`<span class="badge-green">${Q(t)}</span>`:`<span class="badge-gray">${Q(n)}</span>`}function oi(e=`ready`){return e===`ready`?`<span class="badge-green">可创建</span>`:e===`requires_adapter`?`<span class="badge-blue">需适配器</span>`:e===`research_only`?`<span class="badge-gray">调研参考</span>`:`<span class="badge-gray">${Q(e)}</span>`}function si(e=[],t=``){return e.map(e=>`<option value="${Q(e.id)}" ${e.id===t?`selected`:``}>${Q(e.name)} (${Q(e.id)})</option>`).join(``)}function ci(e={},t=[]){return e.requiresKey&&(t.find(t=>(t.allowedPresetIds||[]).includes(e.presetId))||t.find(t=>t.id===`default-${e.vendor}-key-pool`)||t.find(t=>t.vendor===e.vendor&&t.scope===`global`))||null}function li(e={},t=`申请 Key`){return e.credentialUrl?`<a href="${Q(e.credentialUrl)}" target="_blank" rel="noopener noreferrer" class="btn-link">${Q(t)}</a>`:``}function ui(e){return e===`layer`?`组合图层`:e===`dedicated_source`?`专用图源`:`系统图源`}function di(e){return e.visibility?.scope===`external_only`?`专用发布`:`系统图源`}function fi(e,t){let n=window.location.origin,r=e.pathSlug,i=e.auth?.mode===`token`?`?token=您的TOKEN`:``,a=`${n}/api/v1/external/${r}/tilejson${i}`,o=`${n}/api/v1/external/${r}/tilejson.json${i}`,s=`${n}/api/v1/external/${r}/style.json${i}`,c=e.targetType===`source`||e.targetType===`dedicated_source`?(t.tileSources||[]).find(t=>t.id===e.targetId):null;if(c&&ti(c.kind)){let t=`${n}/api/v1/external/${r}/tiles/{z}/{x}/{y}.pbf${i}`,a=`${n}/api/v1/external/${r}.pmtiles${i}`,l=c.kind===`vector-style`?s:c.kind===`vector-tilejson`?o:ni(c.kind)?a:t,u=c.kind===`vector-style`?`const map = new maplibregl.Map({
  container: 'map',
  style: '${s}'
...
`):`// 当前组合图层没有可用图源`;return`
  ...
  `}function pi(e={},t=[],n=[],r=`proxy`){let i=[`fixed`,`pool`].includes(e.mode)?e.mode:`never`;return`
    <div class="form-grid">
  ...
  `}function mi(e,t){let n=Number.isFinite(Number(e))?Math.max(0,Number(e)):t;return{days:Math.floor(n/Vr),hours:Math.floor(n%Vr/Br),minutes:Math.floor(n%Br/zr)}}function hi(e,t,n,r){let i=mi(n,r);return`
    <div class="field-group">
      <label>${t}</label>
  ...
  `}function gi(e={},t=`cache`){return`
    <div class="checkbox-group">
      <input type="checkbox" id="${t}_enabled" name="${t}_enabled" ${e.enabled===!1?``:`checked`}>
      <label for="${t}_enabled">启用服务端缓存</label>
    </div>
    <div class="form-grid" style="margin-top:10px;">
      ${hi(`${t}_ttl`,`缓存有效时间`,e.ttlMs,Lr)}
      ${hi(`${t}_staleTtl`,`软过期时间`,e.staleTtlMs,Rr)}
    </div>
  `}function _i(e,t,n=`proxy`){return{mode:t.get(`${n}_mode`)||`never`,outboundId:t.get(`${n}_outboundId`)||``,poolId:t.get(`${n}_poolId`)||``,fallbackToDirect:!!e.elements[`${n}_fallbackToDirect`]?.checked}}function vi(e,t){let n=Math.max(0,parseInt(e.get(`${t}_days`)||`0`,10)||0),r=Math.max(0,parseInt(e.get(`${t}_hours`)||`0`,10)||0),i=Math.max(0,parseInt(e.get(`${t}_minutes`)||`0`,10)||0);return n*Vr+r*Br+i*zr}function yi(e,t,n=`cache`){return{enabled:!!e.elements[`${n}_enabled`]?.checked,ttlMs:vi(t,`${n}_ttl`),staleTtlMs:vi(t,`${n}_staleTtl`)}}function bi(e,t){return String(e.get(t)||``).split(`,`).map(e=>e.trim()).filter(Boolean)}function xi(e){return{template:String(e.get(`entry_template`)||``).trim(),styleJsonUrl:String(e.get(`entry_styleJsonUrl`)||``).trim(),tileJsonUrl:String(e.get(`entry_tileJsonUrl`)||``).trim(),pmtilesUrl:String(e.get(`entry_pmtilesUrl`)||``).trim(),glyphsUrl:String(e.get(`entry_glyphsUrl`)||``).trim(),spritesUrl:String(e.get(`entry_spritesUrl`)||``).trim()}}function Si(e,t){return{required:!!e.elements.secrets_required?.checked,keyPoolId:t.get(`secrets_keyPoolId`)||``,placement:t.get(`secrets_placement`)||`query`,paramName:t.get(`secrets_paramName`)||`key`}}function Ci(e,t){return{engine:t.get(`rendering_engine`)||`leaflet`,clients:[e.elements.rendering_client_2d?.checked?`2d`:``,e.elements.rendering_client_3d?.checked?`3d`:``].filter(Boolean),fallbackRasterSourceId:t.get(`rendering_fallbackRasterSourceId`)||``}}function wi(e,t){return{attribution:t.get(`license_attribution`)||``,termsUrl:t.get(`license_termsUrl`)||``,officialStatus:t.get(`license_officialStatus`)||`official`,licenseType:t.get(`license_licenseType`)||`unknown`,cacheAllowedByLicense:!!e.elements.license_cacheAllowedByLicense?.checked,publicUseAllowed:!!e.elements.license_publicUseAllowed?.checked,chinaPublicUseReviewed:!!e.elements.license_chinaPublicUseReviewed?.checked,chinaPublicUseRisk:t.get(`license_chinaPublicUseRisk`)||``}}function Ti(e,t){let n=Array.from(e.querySelectorAll(`[data-key-row]`)).map(e=>{let t=t=>e.querySelector(`[name="${t}"]`),n={id:t(`key_id`)?.value||``,alias:t(`key_alias`)?.value||``,enabled:!!t(`key_enabled`)?.checked,secretType:t(`key_secretType`)?.value||`api_key`,placement:t(`key_placement`)?.value||`query`,paramName:t(`key_paramName`)?.value||`key`,priority:parseInt(t(`key_priority`)?.value||`100`,10),weight:parseInt(t(`key_weight`)?.value||`1`,10),qpsLimit:parseInt(t(`key_qpsLimit`)?.value||`0`,10),dailyLimit:parseInt(t(`key_dailyLimit`)?.value||`0`,10),monthlyLimit:parseInt(t(`key_monthlyLimit`)?.value||`0`,10)},r=t(`key_secret`)?.value||``;return r&&(n.secret=r),n});return{id:t.get(`id`),name:t.get(`name`),vendor:t.get(`vendor`),enabled:!!e.elements.enabled?.checked,scope:t.get(`scope`)||`global`,strategy:t.get(`strategy`)||`round_robin`,cooldownMs:parseInt(t.get(`cooldownMs`)||`300000`,10),maxRetriesPerRequest:parseInt(t.get(`maxRetriesPerRequest`)||`2`,10),defaultSecretType:t.get(`defaultSecretType`)||`api_key`,defaultPlacement:t.get(`defaultPlacement`)||`query`,defaultParamName:t.get(`defaultParamName`)||`key`,credentialUrl:String(t.get(`credentialUrl`)||``).trim(),allowedPresetIds:bi(t,`allowedPresetIds`),allowedSourceIds:bi(t,`allowedSourceIds`),keys:n,description:t.get(`description`)||``}}function Ei(e,t){if(!e.editingKeyPool||!t)return;let n=e.editingKeyPool.keys||[],r=new Map(n.filter(e=>e.id).map(e=>[e.id,e])),i=Ti(t,new FormData(t));i.keys=i.keys.map((e,t)=>({...n[t]||{},...r.get(e.id)||{},...e})),e.editingKeyPool={...e.editingKeyPool,...i}}function Di(e={}){let t=e.entry||{};return`
  ...
  `}function Oi(e={},t=[]){let n=e.secrets||{};return`
    <fieldset class="form-card" style="margin-top:15px; padding:15px; background:white;">
  ...
  `}function ki(e={},t=[]){let n=e.rendering||{},r=n.clients||(ti(e.kind)?[`2d`]:[`2d`,`3d`]),i=t.filter(e=>!ti(e.kind)&&!ni(e.kind));return`
    <fieldset class="form-card" style="margin-top:15px; padding:15px; background:white;">
      ...
      <div class="form-grid">
        <div class="field-group">
          ...
          <select name="rendering_engine">
            ${Qr(Jr,n.engine||(ti(e.kind)?`maplibre`:`leaflet`))}
          </select>
        </div>
        <div class="field-group">
  ...
  `}function Ai(e={}){let t=e.license||{};return`
    <fieldset class="form-card" style="margin-top:15px; padding:15px; background:white;">
      ...
      <div class="form-grid">
        <div class="field-group">
          ...
          <select name="license_officialStatus">
            ${Qr([[`official`,`官方`],[`unofficial`,`非官方`],[`community`,`社区`],[`internal`,`内部`]],t.officialStatus||`official`)}
          </select>
        </div>
        <div class="field-group">
          <label>授权类型</label>
          <select name="license_licenseType">
            ${Qr([[`free`,`免费`],[`api-key`,`API Key`],[`commercial`,`商业授权`],[`unknown`,`未知`]],t.licenseType||`unknown`)}
          </select>
        </div>
      </div>
      <div class="checkbox-group" style="margin-top:0;">
  ...
  `}function ji(e={}){let t=Number(e.retina?.normalValue);return Hr[t]?String(t):`1`}function Mi(e,t,n,r,i){let a=e!==!1,o=a?i:r;return`
    <button
      type="button"
      ...
      title="点击切换为${Q(o)}"
    >${Q(a?r:i)}</button>
  `}function Ni(e){let t=String(e||``).toUpperCase();return t===`HIT`?`<span class="badge-green">HIT</span>`:t===`MISS`?`<span class="badge-red">MISS</span>`:t===`BYPASS`?`<span class="badge-gray">BYPASS</span>`:t===`REVALIDATED`?`<span class="badge-blue">REVAL</span>`:t===`STALE`?`<span class="badge-blue">STALE</span>`:t===`ERROR`?`<span class="badge-red">ERROR</span>`:`<span class="badge-gray">${Q(t||`-`)}</span>`}function Pi(e){let t=Number(e.statusCode||0);return t>=200&&t<300?`<span class="badge-green">200 OK</span>`:`<span class="badge-red" title="${Q(e.errorMessage||``)}">${Q(e.statusCode||`-`)}</span>`}function Fi(e){return e.proxyOutboundId?`<span class="badge-blue" title="池: ${Q(e.proxyPoolId||``)}">${Q(e.proxyOutboundId)}</span>`:e.proxyPoolId?`<span class="badge-blue">${Q(e.proxyPoolId)}</span>`:e.proxyConfigured?`<span class="badge-red" title="已配置代理，但本次未命中可用出口">代理未命中</span>`:`<span style="color:#94a3b8;">直连</span>`}async function Ii(e,t,n={}){let{tileSources:r=!1,sourcePresets:i=!1,keyPools:a=!1,mapLayers:o=!1,externalPublishes:s=!1,precacheCatalog:c=!0}=n,l=[],u=[];r&&(l.push(t.listTileSources()),u.push(t=>{e.tileSources=t})),i&&(l.push(t.listSourcePresets()),u.push(t=>{e.sourcePresets=t})),a&&(l.push(t.listKeyPools()),u.push(t=>{e.keyPools=t})),o&&(l.push(t.listMapLayers()),u.push(t=>{e.mapLayers=t})),s&&(l.push(t.listExternalPublishes()),u.push(t=>{e.externalPublishes=t})),c&&(l.push(t.precacheCatalog()),u.push(t=>{e.precacheCatalog=t})),(await Promise.all(l)).forEach((e,t)=>u[t](e))}function Li(e){e.tileSourcesSubTab=e.tileSourcesSubTab||`sources`;let t=[{id:`sources`,label:`图源`},{id:`presets`,label:`图源预设`},{id:`key-pools`,label:`密钥池`},{id:`layers`,label:`图层组合`},{id:`publishes`,label:`发布/API`},{id:`diagnostics`,label:`诊断日志`}],n=``;switch(e.tileSourcesSubTab){case`sources`:n=Ri(e);break;case`presets`:n=zi(e);break;case`key-pools`:n=Vi(e);break;case`layers`:n=Hi(e);break;case`publishes`:n=Ui(e);break;case`diagnostics`:n=Wi(e);break;default:n=`<p>未知子页面</p>`}return`
  ...
  `}function Ri(e){let t=e.tileSources||[],n=e.editingTileSource;if(n){let r=!t.some(e=>e.id===n.id),i=e.proxyOutbounds||[],a=e.proxyPools||[],o=e.keyPools||[],s=ji(n),c=n.category||`custom`,l=n.kind||`xyz-raster`;return`
      <div class="form-card">
        ...
        <form data-tile-sources-form="source">
          ...
          <div class="form-grid">
            ...
            <div class="field-group">
              <label>适配器</label>
              <input name="adapter" value="${Q(n.adapter||(ti(l)?`maplibre-style`:`template`))}" placeholder="template / wmts-kvp / maplibre-style / pmtiles">
            </div>
          </div>
          <div class="form-grid">
            <div class="field-group">
    ...
    <table class="item-table">
      ...
      <tbody>
        ${t.map(t=>{let n=e[`test_source_${t.id}`]||``,r=ii(t);return`
            <tr>
              <td>
              ...
              <td>
                ...
                <span class="badge-gray" style="margin-left:4px;">${Q(di(t))}</span>
              </td>
              <td>
              ...
              <td>
                ${Mi(t.enabled,`source`,t.id,`启用中`,`已禁用`)}
              </td>
              <td>
                <div style="display: flex; flex-direction: column; align-items: flex-start; gap: 4px;">
                  ...
                  ${n&&n!==`loading`&&!n.success?`<span class="test-status test-fail" style="margin-left: 0;" title="${Q(Zr(n))}">失败</span>`:``}
                </div>
              </td>
              <td>
  ...
  `}function zi(e){let t=e.sourcePresets||[],n=e.keyPools||[],r=e.creatingSourceFromPreset;if(r){let e=t.find(e=>e.presetId===r.presetId)||r,i=ci(e,n),a=r.keyPoolId||i?.id||``,o=e.status===`ready`;return`
      <div class="form-card">
        ...
        <form data-tile-sources-form="preset-source">
          ...
          <div class="form-grid">
            ...
            <div class="field-group">
              ...
              ${i?`<p style="color:#64748b; font-size:12px; margin:6px 0 0;">已自动匹配：${Q(i.name)} ${li(i)}</p>`:``}
            </div>
          </div>
    ...
    <table class="item-table">
      ...
      <tbody>
        ${t.map(e=>{let t=ii(e),r=ci(e,n);return`
            <tr>
              <td>
                <strong>${Q(e.name)}</strong>
              ...
              <td>
                ...
                ${r?`<div style="margin-top:4px;"><span class="badge-gray">${Q(r.name)}</span> ${li(r)}</div>`:``}
              </td>
              <td>${oi(e.status)}</td>
              <td>
                ${ai(e.cacheAllowedByLicense!==!1,`可缓存`,`禁缓存`)}
                ${ai(!!e.publicUseAllowed,`可公开`,`慎公开`)}
              </td>
              <td>
  ...
  `}function Bi(e,t=!1,n={}){return(e.keys||[]).map((r,i)=>{let a=n[`test_key_${e.id}_${r.id}`]||``,o=!r.hasSecret&&!r.maskedPreview,s=r.secretType||e.defaultSecretType||`api_key`,c=r.placement||e.defaultPlacement||`query`,l=r.paramName||e.defaultParamName||`key`;return`
      <div class="key-row" data-key-row="${i}">
        <div class="key-row-main">
          ...
          <select name="key_secretType">
            ${$r(s,[[`api_key`,`api_key`],[`token`,`token`],[`tk`,`tk`],[`ak`,`ak`],[`appid`,`appid`]])}
            ${Qr([[`api_key`,`api_key`],[`token`,`token`],[`tk`,`tk`],[`ak`,`ak`],[`appid`,`appid`]],s)}
          </select>
        </div>
        <div class="key-row-main">
        ...
        <div class="flex-actions">
          ...
          ${a&&a!==`loading`&&!a.success?`<span class="test-status test-fail" title="${Q(Zr(a))}">不可用</span>`:``}
        </div>
      </div>
    `}).join(``)||`<p style="color:#64748b;">暂无 Key，请添加至少一个 Key。</p>`}function Vi(e){let t=e.keyPools||[],n=e.sourcePresets||[],r=e.tileSources||[],i=e.editingKeyPool;if(i){let a=!t.some(e=>e.id===i.id);return`
      <div class="form-card">
          ...
          <div class="form-grid">
            <div class="field-group">
              ...
              <select name="defaultSecretType">
                ${$r(i.defaultSecretType||`api_key`,[[`api_key`,`api_key`],[`token`,`token`],[`tk`,`tk`],[`ak`,`ak`],[`appid`,`appid`]])}
                ${Qr([[`api_key`,`api_key`],[`token`,`token`],[`tk`,`tk`],[`ak`,`ak`],[`appid`,`appid`]],i.defaultSecretType||`api_key`)}
              </select>
            </div>
            <div class="field-group">
              ...
              <div style="display:grid; grid-template-columns: 1fr 1fr; gap:8px;">
                <select name="defaultPlacement">
                  ${Qr(Gr,i.defaultPlacement||`query`)}
                </select>
        ...
        ${t.map(t=>{let n=e[`test_key_pool_${t.id}`]||``,r=(t.keys||[]).filter(e=>e.enabled!==!1).length;return`
            <tr>
              <td>
                <strong>${Q(t.name)}</strong>
                <div style="color:#64748b; font-size:11px; margin-top:2px;">${Q(t.id)}</div>
                ${t.credentialUrl?`<div style="margin-top:4px;">${li(t,`申请 / 管理 Key`)}</div>`:``}
              </td>
              <td>
                <span class="badge-gray">${Q(t.vendor)}</span>
                <span class="badge-blue" style="margin-left:4px;">${Q(Kr.find(([e])=>e===t.strategy)?.[1]||t.strategy)}</span>
              ...
              <td>
                ...
                ${n&&n!==`loading`&&!n.success?`<span class="test-status test-fail" title="${Q(Zr(n))}">失败</span>`:``}
              </td>
              <td>${Mi(t.enabled,`key-pool`,t.id,`启用中`,`已禁用`)}</td>
              <td>
                <div class="flex-actions">
  ...
  `}function Hi(e){let t=e.mapLayers||[],n=e.editingMapLayer;if(n){let r=!t.some(e=>e.id===n.id),i=e.tileSources||[],a=n.items||[{sourceId:``,opacity:1,zIndex:0}];return`
      <div class="form-card">
      ...
      <tbody>
        ${t.map(t=>`
          <tr>
            ...
            <td>
              ${Mi(t.enabled,`layer`,t.id,`启用中`,`已禁用`)}
            </td>
            <td>
  ...
  `}function Ui(e){let t=e.externalPublishes||[],n=e.editingExternalPublish,r=e.selectedPublishId||t[0]?.id||``;if(n){let r=!t.some(e=>e.id===n.id),i=e.tileSources||[],a=i.filter(e=>e.visibility?.scope!==`external_only`&&e.permissions?.externalApiAllowed!==!1),o=i.filter(e=>e.visibility?.scope===`external_only`),s=e.mapLayers||[],c=e.proxyOutbounds||[],l=e.proxyPools||[],u=n.targetType||`source`,d=u===`layer`?s.map(e=>`<option value="${e.id}" ${n.targetId===e.id?`selected`:``}>${Q(e.name)} (${e.id})</option>`).join(``):u===`dedicated_source`?o.map(e=>`<option value="${e.id}" ${n.targetId===e.id?`selected`:``}>${Q(e.name)} (${e.id} / ${ei(e.kind)})</option>`).join(``):a.map(e=>`<option value="${e.id}" ${n.targetId===e.id?`selected`:``}>${Q(e.name)} (${e.id} / ${ei(e.kind)})</option>`).join(``),f=n.overrides?.proxy||null,p=n.overrides?.cache||null;return`
      <div class="form-card">
        ...
        <form data-tile-sources-form="publish">
          ...
          <div class="form-grid">
            ...
            <div class="field-group">
              ...
              <select name="targetId" required>
                <option value="">请选择${Q(ui(u))}</option>
                ${d}
              </select>
            </div>
          </div>
          ...
          <fieldset class="form-card" style="margin-top:15px; padding:15px; background:white;">
            ...
            ${pi(f||{mode:`never`},c,l,`publish_proxy`)}
          </fieldset>
    ...
    `}let i=e.lastGeneratedToken?`<div class="admin-token-notice">
       ...
       </div>`:``,a=t.find(e=>e.id===r),o=``;return a&&(o=fi(a,e)),`
    ${i}
      ...
      <tbody>
        ${t.map(t=>{let n=e[`test_publish_${t.id}`]||``;return`
            <tr style="background: ${t.id===r?`#f0fdfa`:`transparent`}; cursor:pointer;" data-tile-sources-select-publish="${t.id}">
              ...
              <td style="white-space: nowrap;"><span class="badge-gray">${Q(ui(t.targetType))}</span></td>
              ...
              <td style="white-space: nowrap;">
                ${Mi(t.enabled,`publish`,t.id,`已发布`,`已禁用`)}
              </td>
              <td style="white-space: nowrap;">
    ...
    ${o}
  `}function Wi(e){e.diagnosticsLogType=e.diagnosticsLogType||`source`;let t=e.diagnosticsPublishId||``,n=e.diagnosticsSourceId||``,r=e.externalPublishes||[],i=e.tileSources||[],a=e.diagnosticsLogType===`source`,o=a?e.sourceAccessLogs||[]:e.diagnosticLogs||[],s=a?e.sourceAccessLogsError||``:e.diagnosticLogsError||``;return`
    <div class="admin-panel-head">
      <h3>运行诊断日志</h3>
      <div style="display:flex; gap:10px; align-items:center;">
    ...
    `}else a=`
            ...
            ${t.map(t=>{let n=e[`test_outbound_${t.id}`]||``;return`
                <tr>
                  ...
                  <td>
                    ...
                    ${n&&n!==`loading`&&!n.success?`<span class="test-status test-fail" title="${Q(Qi(n))}">失败: ${Q(Qi(n))}</span>`:``}
                  </td>
                  <td>
      ...
      <div>
        ...
        <table class="item-table">
          ...
          <tbody>
            ${n.map(t=>{let n=e[`test_pool_${t.id}`]||``,r=n&&n!==`loading`?$i(n):null;return`
                <tr>
                  <td>
                    <strong>${Q(t.name)}</strong>
                    <div style="color: #64748b; font-size:11px; margin-top:2px;">${Q(t.id)}</div>
  ...
  `}function fa(e){e.root.innerHTML=`
  ...
  `}function pa(e,t){e.root.innerHTML=`
    <section class="admin-shell">
      ...
      <div class="admin-layout">
        <nav class="admin-tabs" aria-label="后台导航">
          ${aa.map(t=>`
            <a href="${oa(t.id)}" data-admin-tab="${t.id}" class="${e.activeTab===t.id?`is-active`:``}">
              ${Q(t.label)}
            </a>
          `).join(``)}
        </nav>
  ...
  `}var $={root:null,activeTab:`overview`,loading:!1,message:``,error:``,session:null,system:null,cache:null,cacheLoading:!1,cacheError:``,visits:null,visitsLoading:!1,visitsError:``,settings:null,tasks:[],kmls:[],precacheForm:{providerId:``,bounds:{west:113.24,south:23.11,east:113.29,north:23.15},minZoom:12,maxZoom:12,concurrency:4,requestIntervalMs:0,refresh:!1},precacheEstimate:null,precacheEstimateStatus:``,precacheEstimateError:``,expandedTaskIds:new Set,amapLoader:null,AMap:null,map:null,rectangle:null,precacheMapHeight:260},ma=null;function ha(e){ma=e}function ga(e=``,t=``){$.message=e,$.error=t,ma&&ma(e,t)}function _a(e){ca(e)&&($.activeTab=e)}var va=null;ha((e,t)=>{va&&=(clearTimeout(va),null);let n=t||e;n&&n!==`正在加载`&&n!==`正在登录`&&(va=setTimeout(()=>{ga(``),document.querySelector(`[data-admin-login]`)?fa($):Sa()},4e3))});function ya(){return ua(window.location)}function ba(e){window.history.replaceState(null,``,`${oa(e)}${window.location.hash}`)}function xa(){return sa($.activeTab).render($)}function Sa(){pa($,xa()),sa($.activeTab).afterRender?.($,Z)}window.renderDashboard=Sa;function Ca(e){return{api:Z,event:e,renderDashboard:Sa,setNotice:ga,showCheckboxConfirm:Se,showConfirm:xe,state:$}}function wa(...e){(!e.length||e.includes($.activeTab))&&Sa()}async function Ta(e,t){let n=sa($.activeTab)[e];return n instanceof Function?!!await n(Ca(t)):!1}async function Ea(e={}){let t=!!e.cacheOnly;Object.assign($,{cacheLoading:!0,cacheError:``,visitsLoading:t?$.visitsLoading:!0,visitsError:t?$.visitsError:``}),wa(`overview`,`cache`),Z.cache().then(e=>{$.cache=e,$.cacheError=``,e.refreshing&&window.setTimeout(()=>Ea({cacheOnly:!0}),1500)}).catch(e=>{$.cacheError=e.message}).finally(()=>{$.cacheLoading=!1,wa(`cache`)}),!t&&Z.visits().then(e=>{$.visits=e,$.visitsError=``}).catch(e=>{$.visitsError=e.message}).finally(()=>{$.visitsLoading=!1,wa(`overview`)})}async function Da(){$.loading=!0,ga(`正在加载`),Sa();try{let[e,t,n,r,i,a,o,s,c,l,u,d,f]=await Promise.all([Z.session(),Z.system(),Z.settings(),Z.tasks(),Z.kmls(),Z.listTileSources(),Z.listSourcePresets(),Z.listKeyPools(),Z.listMapLayers(),Z.listProxyOutbounds(),Z.listProxyPools(),Z.listExternalPublishes(),Z.precacheCatalog()]);Object.assign($,{session:e,system:t,settings:n,tasks:r,kmls:i,tileSources:a,sourcePresets:o,keyPools:s,mapLayers:c,proxyOutbounds:l,proxyPools:u,externalPublishes:d,precacheCatalog:f,loading:!1}),ga(``),Sa(),sa($.activeTab).afterLoad?.($,Z),Ea()}catch(e){$.loading=!1,e.status===401?(wn(),ga(``,e.message),fa($)):(ga(``,e.message),Sa())}}async function Oa(e){let t=e.target.closest(`[data-admin-login]`);if(t){e.preventDefault(),ga(`正在登录`),fa($);try{await Cn({username:t.elements.username.value,password:t.elements.password.value}),ga(``),await Da()}catch(e){ga(``,e.message),fa($)}return}await Ta(`handleSubmit`,e)}async function ka(e){let t=e.target.closest(`[data-admin-tab]`);if(t){e.preventDefault(),_a(t.getAttribute(`data-admin-tab`)),ba($.activeTab),Sa(),sa($.activeTab).afterEnter?.($,Z);return}let n=e.target.closest(`[data-admin-action]`);if(n){let e=n.getAttribute(`data-admin-action`);if(e===`logout`){wn(),ga(``),fa($);return}if(e===`refresh`){await Da();return}if(e===`close-notice`){ga(``),document.querySelector(`[data-admin-login]`)?fa($):Sa();return}}await Ta(`handleClick`,e)}async function Aa(e){await Ta(`handleChange`,e)}async function ja(e={}){if(document.body.classList.add(`admin-view`),_a(ya()),ba($.activeTab),$.amapLoader=e.amapLoader||null,$.root=document.getElementById(`admin-root`),$.root.hidden=!1,$.root.addEventListener(`submit`,Oa),$.root.addEventListener(`click`,ka),$.root.addEventListener(`change`,Aa),$.root.addEventListener(`input`,Aa),!xn()){fa($);return}await Da()}function Ma(){`serviceWorker`in navigator&&window.addEventListener(`load`,()=>{navigator.serviceWorker.register(`/sw.js`).catch(e=>{console.warn(`Service Worker 注册失败`,e)})})}async function Na(e){let{init:t,title:n=`私有地图服务`,message:r=`管理员启用了访问控制，请输入密码解锁`,submitText:i=`载入地图`,loadingText:a=`正在验证...`}=e;try{(await Tn()).required?Pa({init:t,title:n,message:r,submitText:i,loadingText:a}):await t()}catch(e){console.error(`Failed to check map access status`,e),Pa({init:t,title:n,message:`访问状态检查失败，请稍后重试`,submitText:i,loadingText:a,allowRetry:!0})}}function Pa(e){let{init:t,title:n,message:r,submitText:i,loadingText:a,allowRetry:o=!1}=e;document.getElementById(`map-lock-screen`)?.remove();let s=document.createElement(`div`);s.id=`map-lock-screen`,s.className=`lock-screen-backdrop`,s.innerHTML=`
  ...
  `,document.body.appendChild(s);let c=document.getElementById(`lock-screen-form`),l=document.getElementById(`lock-screen-error`);s.querySelector(`[data-lock-retry]`)?.addEventListener(`click`,()=>{s.remove(),Na({init:t,title:n,submitText:i,loadingText:a})}),c.addEventListener(`submit`,async e=>{e.preventDefault(),l.style.display=`none`;let n=c.elements.password.value.trim();if(!n)return;let r=c.querySelector(`button[type="submit"]`);try{r.disabled=!0,r.textContent=a,await En(n),s.remove(),await t()}catch(e){r.disabled=!1,r.textContent=i,l.textContent=e.message||`访问密码错误`,l.style.display=`block`}})}export{te as $,bt as A,xe as B,ft as C,ut as D,dt as E,Ke as F,O as G,_e as H,tt as I,ae as J,A as K,qe as L,V as M,rt as N,xt as O,Je as P,h as Q,be as R,Ct as S,yt as T,he as U,Ce as V,oe as W,w as X,ie as Y,m as Z,wt as _,Q as a,u as at,Ot as b,Qt as c,Mt as d,v as et,W as f,at as g,it as h,la as i,s as it,pt as j,_t as k,ln as l,At as m,Ma as n,d as nt,_n as o,l as ot,jt as p,j as q,ja as r,o as rt,vn as s,Na as t,f as tt,Pt as u,H as v,lt as w,St as x,gt as y,we as z};