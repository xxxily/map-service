...
test('KML media thumbnails open the in-app preview instead of a blank browser page', () => {
  ...
  assert.match(stylesSource, /\.media-preview-iframe-shell\s*\{[^}]*align-items:\s*center;[^}]*justify-content:\s*center;[^}]*background:\s*var\(--media-preview-surface,\s*#050909\);/s)
  assert.match(stylesSource, /\.media-preview-iframe\s*\{[^}]*background:\s*var\(--media-preview-surface,\s*#050909\);[^}]*color-scheme:\s*dark;/s)
})
