...
import { getKmlLeafletPerformanceOptions } from '../src/map/kml-performance.js'

const layersSource = readFileSync(new URL('../src/map/layers.js', import.meta.url), 'utf8')
const mainSource = readFileSync(new URL('../src/main.js', import.meta.url), 'utf8')
const performanceSource = readFileSync(new URL('../src/map/kml-performance.js', import.meta.url), 'utf8')
...
test('无缝瓦片立即变为不透明，同时保留缩放期间的旧层级兜底', () => {
  const mapOptions = getKmlLeafletPerformanceOptions()
...
test('空间受限分享的底图目录失败时不允许使用普通地图兜底，并关闭世界横向环绕', () => {
  ...
  assert.match(mainSource, /minZoom: restrictedShare \? shareSpatial\.effectiveMinZoom/)
  assert.match(mainSource, /maxBounds: restrictedBounds \|\| undefined/)
