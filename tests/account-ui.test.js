...
test('资源集合来源选择器使用纵向选项布局，资源项标题可选而地址必填', () => {
  ...
  const map3dSource = fs.readFileSync(path.join(projectRoot, 'src/map3d/kml.js'), 'utf8')
  ...
  assert.match(source, /hasLiveBinding/)
  assert.doesNotMatch(source, /seenCollectionIds/)
  ...
  assert.match(map3dSource, /isKmlResourceCollectionStatusFeature\(feature\)/)
  assert.match(map3dSource, /delete editedFeature\.resourceCollectionStatus/)
  assert.match(appSource, /function resourceCollectionBatchLimit \(\)/)
  assert.match(appSource, /index \+= batchLimit/)
