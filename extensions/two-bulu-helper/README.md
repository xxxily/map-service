...
扩展目录有代码更新时，打开 `chrome://extensions/`，在“map-service 两步路导入助手”卡片上点击“重新加载”，随后刷新 map-service 和已打开的两步路页面。当前版本为 `0.4.0`。除单条轨迹导入外，支持从公开用户轨迹列表读取可见条目并由网站按筛选条件串行导入。扩展图标采用“轨迹双节点 + 导入箭头”识别符号，资源位于 `icons/`，如需重新生成尺寸可从 `two-bulu-helper.svg` 导出。
